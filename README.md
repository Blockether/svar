...
;; deps.edn
{:deps {com.blockether/svar {:mvn/version "0.7.144"}}}
```
...
The session keeps one Responses WebSocket, a stable `prompt_cache_key`, and the latest `previous_response_id`. Before its first inference it sends a best-effort `response.create` prewarm with `generate=false`; the real request then reuses that response ID and sends an empty delta (`:websocket-prewarm? false` disables this optimization). Afterward it sends only new user/tool items. A turn may select another model on the same Codex provider with `{:input ... :routing {:model "..."}}`: Svar retains a healthy physical WebSocket, resets the model-bound continuation, and replays canonical history once; later turns on that model are incremental again. Provider changes require a different session. At the next turn boundary after 55 minutes, Svar gracefully rotates the physical socket before Codex's 60-minute limit and fully replays canonical history with the same `prompt_cache_key`; `:websocket-max-age-ms` changes that threshold.

Svar also owns Codex's opaque `x-codex-turn-state`. It captures the first value returned for a logical agent/tool turn, echoes it through that turn's tool-result follow-ups (`client_metadata` on WebSocket, the request header on HTTP), and clears it before the next user turn, history replacement, or model change. Callers neither store nor forward this provider state. `session-status` exposes a sanitized transport view instead: current mode and connection age plus counters for socket opens, prewarms, initial requests, delta requests, full replays, reconnects, cursor/history/model resets, HTTP fallbacks, oversized one-turn HTTP requests, and rotations. It also carries the session-scoped provider `:prompt-cache` summary described above. It never returns the socket, cursor, turn-state token, credentials, cache key, or learned payload ceiling.

Svar still owns canonical history. The JDK WebSocket cannot negotiate Codex's `permessage-deflate`, so a cursorless request larger than `:websocket-max-full-request-bytes` (default 262144 UTF-8 bytes) goes directly through HTTP/SSE for that turn instead of spending a deterministic reconnect ladder. This does not permanently disable WebSocket: folding or replacing history with a request below the boundary starts a fresh WebSocket chain. If the server explicitly closes a request with RFC 6455 status `1009`, Svar learns a lower session-local boundary and likewise retries that turn once through HTTP rather than replaying the same frame on five sockets. HTTP response IDs are never carried into WebSocket because Codex continuation IDs are transport-bound.

For other terminal WebSocket errors, Svar aborts the physical stream immediately. A rejected cursor opens a fresh socket and replays the whole history, while a dropped connection uses the same fresh-socket replay up to `:websocket-max-retries` times (default 5, waits tuned by `:websocket-retry-delay-ms`). When that ladder is spent - or the endpoint refuses the upgrade with `426` - the session leaves WebSocket for the rest of its life and runs on HTTP/SSE with full history. A restarted turn tells `:on-chunk` so consumers can drop text from an interrupted attempt: `{:event/type :llm.session/stream-restarted :restarted? true :content ""}`; close-frame restarts additionally expose non-sensitive `:close-status` and `:close-reason`. Informational `codex.rate_limits` events are normalized into the result’s `:rate-limits` and emitted through `:on-chunk` as `{:event/type :llm.session/rate-limits :rate-limits …}`. Calls on one session are serialized. The caller's `:cancel-fn` is observed even while a completely quiet socket is waiting; cancellation aborts that socket and surfaces `:svar.core/stream-cancelled` without reconnect or HTTP fallback. `with-open` or `close-session!` closes an idle socket gracefully and aborts an active one. `session-history` returns the replayable canonical messages and `session-status` returns Svar's live observability snapshot.
...
translations. GLM-5.2 sells `high`/`max`; GLM-5.3 adds the light `low` rung, and
both send `{:thinking {:type "enabled"} :reasoning_effort "<rung>"}`. Routed
