...
(ns reproduce_eof
  (:require [com.blockether.svar.internal.rlm.internal.pageindex.vision]
...
(defn -main
  ...
  [& args]
  (let [pdf-path "resources-test/chapter.pdf"
        model "glm-4.6v"
        config (svar/make-config)
        max-attempts 5]
    ...
    (loop [attempt 1]
      ...
      (try
        (let [result (vision/extract-text-from-image-pdf pdf-path
                                     {:model model
                                      :timeout-ms 480000
                                      :parallel 1})]
          _ (println "Result:" (pr-str result)))
        (catch Exception e
