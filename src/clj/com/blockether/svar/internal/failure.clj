...
(def ^:private GATEWAY_TRANSIENT_PATTERNS
  ...
  ["litellm\\.timeout" "litellm\\.apiconnectionerror" "litellm\\.apierror"
   ...
   "litellm\\.ratelimiterror" "litellm\\.middlewareerror"
   "apiconnectionerror" "timeout error" "timed out" "timeout passed="
   "read timeout" "connect timeout" "connection timeout"
   "no deployments available" "no healthy deployment"
   "deployments are cooling down" "no fallback model group found"
   "bad gateway" "gateway time.?out" "upstream request timeout"
   "temporarily unavailable" "try again later" "please try again"
   "capacity" "throttlingexception" "modelnotreadyexception"
...
(def ^:private PRE_RESPONSE_DROP_PATTERNS
  ...
  ["header parser received no bytes" "received no bytes"
   "empty reply from server" "server disconnected"
   "remote end closed connection" "connection reset by peer"
   "connection reset" "connection aborted" "socket hang up"
   "premature close" "peer closed connection" "eof occurred"
   "stream closed before response" "unexpected end of stream"
   "goaway" "connection closed before"])
...
(def RETRYABLE_TRANSIENT_MESSAGE_PATTERN
  ...
  (re-pattern
    (str/join "|" (concat GENERIC_TRANSIENT_PATTERNS
                    GATEWAY_TRANSIENT_PATTERNS
                    PRE_RESPONSE_DROP_PATTERNS))))

(def ^:private PRE_RESPONSE_DROP_PATTERN
  (re-pattern (str/join "|" PRE_RESPONSE_DROP_PATTERNS)))
...
(defn- definitive-client-error?
  ...
  (boolean
    (and status
      (<= 400 (long status) 499)
      (not (contains? #{408 425 429} (long status))))))
...
(def ^:private RESOURCE_MISMATCH_PATTERNS
  ...
  ["created under a different azure openai resource"
   "use the same resource that created the item"
...
(def ^:private TOOL_SCHEMA_PATTERNS
  ...
  ["extra inputs are not permitted" "additionalproperties"
   "unrecognized request argument" "unknown field"
   "unexpected keyword argument" "is not permitted"])
...
(def ^:private CONTEXT_LENGTH_PATTERNS
  ["context length" "context_length_exceeded" "maximum context"
   ...
   "reduce the length of the messages" "exceeds the maximum"
...
(defn request-id
  ...
  (let [data (ex-data e)
        ...
        header-id (when (map? headers)
                    (some (fn [k] (get headers k))
                      ["x-request-id" "X-Request-Id" "x-litellm-call-id"
...
(defn retry-after-ms
  ...
  (let [headers (:headers (ex-data e))
        raw (when (map? headers)
              (some (fn [k] (get headers k))
                ["retry-after" "Retry-After" :retry-after :Retry-After]))]
    (when raw
      (try
        (let [secs (Long/parseLong (str/trim (str raw)))]
...
(defn backoff-ms
  ...
  ([base max-ms rand-fn]
   (let [capped (min (double base) (double max-ms))
         half (/ capped 2.0)]
  ...
  (loop [t e n 0]
    (cond
      ...
      (or (instance? java.net.ConnectException t)
        ...
        (instance? java.nio.channels.UnresolvedAddressException t)) true
...
(defn classify
  ...
  (let [data (ex-data e)
    ...
    (cond
      ...
      (contains? #{401 403} (some-> status long))
      (answer :auth false false
        "the provider rejected the credentials"
        "check the API key/token for this provider")

      (any-substring? hay RESOURCE_MISMATCH_PATTERNS)
      (answer :resource-mismatch false true
      ...
      (and (any-substring? hay TOOL_SCHEMA_PATTERNS)
        (str/includes? hay "tool"))
      (answer :tool-schema-unsupported false true
      ...
      (and (any-substring? hay CONTEXT_LENGTH_PATTERNS)
        (definitive-client-error? status))
      (answer :context-length-exceeded false true
      ...
      (answer :model-unavailable false false
      ...
      (answer :stream-interrupted (not started?) (not started?)
      ...
      (answer :connect-timeout true false
      ...
      (answer :transport-drop true false
      ...
      (= 429 (some-> status long))
      (answer :rate-limited true false
        "the provider is throttling this key"
        "retry after the cooldown; sustained throttling means switching provider")

      (or (contains? #{408 425 504 522 524 598 599} (some-> status long))
        (str/includes? hay "timeout"))
      (answer :upstream-timeout true nil
      ...
      (boolean
        (or
          ...
          (and (instance? clojure.lang.ExceptionInfo e)
            (some-> cause transport-retryable?)))))))
...
(defn transient-error?
  ...
  ([^Throwable e opts]
   (let [data      (ex-data e)
         status    (:status data)
         etype     (:type data)
         codes     (or (:transient-status-codes opts) TRANSIENT_STATUS_CODES)
         msg       (ex-message e)
