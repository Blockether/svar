...
(def KNOWN_MODEL_METADATA
  ...
  {;; ── OpenAI GPT-4o ────────────────────────────────────────────────────────
   ...
   "glm-5.3" {:intelligence :high
              ...
              :reasoning? true
              :reasoning-style :zai-effort}
   "glm-5.3-flash" {:intelligence :high
                    :speed :fast
                    :capabilities #{:chat :vision}
                    :reasoning? true
                    :reasoning-style :zai-effort}
...
(def KNOWN_PROVIDER_MODELS
  ...
  {:openai {;; Long-context tier pricing (>272k tokens) — not in models.dev.
   ...
   {"glm-4.6" {:context 200000 :json-object-mode? true}
    ...
    ;; 2026-08-26 models.dev catalog has only gateway rows — no retail `zai`
    ;; row. Use Z.ai's stable list rates; its half-price launch promotion ends
    ;; 2026-09-09 and must not leave long-lived budget accounting underpriced.
...
(def DEFAULT_FIRST_BYTE_TIMEOUT_MS
