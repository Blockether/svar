...
(def ^:private duration-unit-ms {"ms" 1 "s" 1000 "m" 60000 "h" 3600000 "d" 86400000})
...
(defn- anthropic
  [headers _now-ms]
  (let [u-reset
        (parse-absolute-ms (header headers "anthropic-ratelimit-unified-reset"))

        u-remain
        (->long (header headers "anthropic-ratelimit-unified-remaining"))

        u-status
        (header headers "anthropic-ratelimit-unified-status")

        r-reset
        (parse-absolute-ms (header headers "anthropic-ratelimit-requests-reset"))

        r-remain
        (->long (header headers "anthropic-ratelimit-requests-remaining"))

        r-limit
        (->long (header headers "anthropic-ratelimit-requests-limit"))

        t-reset
        (parse-absolute-ms (header headers "anthropic-ratelimit-tokens-reset"))

        t-remain
        (->long (header headers "anthropic-ratelimit-tokens-remaining"))

        t-limit
        (->long (header headers "anthropic-ratelimit-tokens-limit"))
        ...
        (cond-> {}
          ...
          (assoc :unified
            (cond-> {}
              u-reset
              (assoc :resets-at-ms u-reset)

              u-remain
              (assoc :remaining u-remain)

              u-status
              (assoc :status u-status)))

          (or r-reset r-remain r-limit)
          (assoc :requests
            (cond-> {}
              r-reset
              (assoc :resets-at-ms r-reset)

              r-remain
              (assoc :remaining r-remain)
          ...
          (assoc :tokens
            (cond-> {}
              t-reset
              (assoc :resets-at-ms t-reset)

              t-remain
              (assoc :remaining t-remain)
    ...
    (when (seq windows)
      (cond-> {:windows windows}
        (soonest u-reset r-reset t-reset)
        (assoc :resets-at-ms (soonest u-reset r-reset t-reset))

        (or u-remain r-remain)
        (assoc :remaining (or u-remain r-remain))
...
(defn- openai
  [headers now-ms]
  (let [r-reset-ms
        (parse-duration-ms (header headers "x-ratelimit-reset-requests"))

        r-remain
        (->long (header headers "x-ratelimit-remaining-requests"))

        r-limit
        (->long (header headers "x-ratelimit-limit-requests"))

        t-reset-ms
        (parse-duration-ms (header headers "x-ratelimit-reset-tokens"))

        t-remain
        (->long (header headers "x-ratelimit-remaining-tokens"))

        t-limit
        (->long (header headers "x-ratelimit-limit-tokens"))

        r-reset
        (when r-reset-ms (+ (long now-ms) (long r-reset-ms)))

        t-reset
        (when t-reset-ms (+ (long now-ms) (long t-reset-ms)))
    ...
    (when (seq windows)
      (cond-> {:windows windows}
        (soonest r-reset t-reset)
        (assoc :resets-at-ms (soonest r-reset t-reset))
          ...
          (filter (fn [[k _]]
                    (and (string? k)
                         (let [lk (str/lower-case k)]
                           (or (str/includes? lk "ratelimit") (str/includes? lk "rate-limit")))))
                  headers))))
...
(defn parse
  ...
  ([api-style headers] (parse api-style headers (System/currentTimeMillis)))
  ([api-style headers now-ms]
   (when (map? headers)
     (let [base
           (case api-style
             :anthropic
             (anthropic headers now-ms)
