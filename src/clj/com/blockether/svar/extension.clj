(ns com.blockether.svar.extension
  ...
  (:require [com.blockether.svar.internal.jsonish :as jsonish]
...
(defn- entry-ts [entry] (long (if (map? entry) (:ts entry) entry)))
...
(defn- sum-token-window [entries] (long (reduce + 0 (map #(long (or (:n %) 0)) entries))))
...
(defn- budget-limits
  ...
  (when (:budget router)
    (let [limit
      ...
      (cond-> {:limit limit :spent spent}
        max-tokens
        (assoc-in [:remaining :tokens] (max 0 (- (long max-tokens) (long (:total-tokens spent 0)))))
...
(defn provider-limits
  ...
  [router]
  (let [state
        (or (some-> (:state router)
                    deref)
            {})

        providers
        (reduce (fn [acc provider]
                  (let [pid
                        (:id provider)
                        ...
                        requests
                        (prune-window router (:requests ps))

                        tokens
                        (prune-window router (:tokens ps))]

                    (assoc acc
                      pid {:windowed {:requests (long (count requests))
    ...
    (cond-> {:window-ms (:window-ms router 60000) :providers providers}
      (:budget router)
      (assoc :budget (budget-limits router)))))
...
(defn parse-diagnose
  ...
  ([text] (parse-diagnose text nil))
  ([text spec-def]
   (try (let [{:keys [value warnings] :as parsed} (jsonish/parse-json text)]
          (if spec-def
            (try (let [result (spec/str->data-with-spec text spec-def)
            ...
            {:ok? true :phase :parse :parse parsed :value value :warnings warnings}))
        (catch Throwable e {:ok? false :phase :parse :error (exception->data e)}))))
