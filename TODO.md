...
- [x] Treat close status 1009 as a deterministic one-turn HTTP bridge and preserve later WebSocket re-entry; keep `:websocket-max-full-request-bytes` as an explicit ceiling only.
- [x] Remove the unverified 262144-byte default HTTP bypass. Live JDK WebSocket probes (2026-09-09) with uncompressed 300 KB, 1.5 MB and 8 MB requests reached provider model validation instead of a 1009 size rejection; these probes did not test model generation.
