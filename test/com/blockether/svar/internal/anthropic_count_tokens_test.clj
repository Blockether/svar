...
(defdescribe
  check-context-limit-input-override-test
  (describe ":input-tokens override (priority 1)"
            ...
            (it "a huge override flips the overflow verdict"
                (let [r (router/check-context-limit "claude-opus-4-8"
                                                    msgs
                                                    {:input-tokens 5000000
            ...
            (it "an explicit :input-tokens wins over :exact-count-fn (no refine call)"
                (let [calls
                      ...
                      r
                      (router/check-context-limit "claude-opus-4-8"
                                                  msgs
                                                  {:input-tokens 42
...
(defdescribe
  check-context-limit-refine-test
...
(defdescribe
  ...
  (describe "happy path"
            ...
            (it "collapses a trailing slash in base-url (no double slash)"
                (let [captured (atom nil)]
                  (with-redefs-fn {#'llm/http-post! (fn [url _body _headers _t]
                                                      (reset! captured url)
  ...
  (describe "failure modes return nil (never throw)"
            (it "non-200 → nil"
                (expect (nil? (with-redefs-fn {#'llm/http-post! (fn [_ _ _ _]
...
(defdescribe
  anthropic-exact-count-fn-gating-test
  (describe
    "supplies a thunk only for direct Anthropic"
    (it ":anthropic → returns a working thunk"
        (let [f (anthropic-exact-count-fn
                  msgs
                  "claude-opus-4-8"
                  {:api-style :anthropic :api-key "k" :base-url "https://api.anthropic.com/v1"})]
          (expect (fn? f))
          (expect (= 7
                     (with-redefs-fn {#'llm/http-post! (fn [_ _ _ _]
                                                         {:parsed {"input_tokens" 7} :status 200})}
                       (fn []
                         (f)))))))
    (it ":openai-compatible-chat → nil (proxied Claude has no count_tokens)"
        (expect (nil? (anthropic-exact-count-fn msgs
                                                "claude-opus-4.8"
