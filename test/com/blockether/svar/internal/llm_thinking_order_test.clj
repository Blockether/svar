...
(defdescribe
  demote-interior-thinking-blocks-test
  ...
  (describe "interior thinking demotion"
            (it "demotes a trailing thinking that follows a text block"
                ...
                (let [in
                      [{:type "thinking" :thinking "first" :thinking-signature "sig-A"}
            ...
            (it "demotes EVERY thinking block after the first non-thinking block"
                (let [in
                      [{:type "thinking" :thinking "lead" :thinking-signature "sig"}
                       {:type "text" :text "body"}
  ...
  (describe "no-op cases"
            ...
            (it "passes through a text-only message"
                (let [in [{:type "text" :text "hi"}]]
                  (expect (= in (demote in)))))
            (it "is identity on []" (expect (= [] (demote []))))))
...
(defdescribe
  anthropic-wire-body-thinking-order-test
  (it "wire body for [thinking, text, thinking] demotes the trailing thinking"
      (let [messages
            [{:role "user" :content "redo it"}
             {:role "assistant"
              :content [{:type "thinking" :thinking "plan A" :thinking-signature "sig-A"}
                        {:type "text" :text "first answer"}
                        {:type "thinking" :thinking "plan B" :thinking-signature "sig-B"}]}
             {:role "user" :content "and now follow-up"}]

            body
            (build-body messages "claude-opus-4-7" {:max_tokens 1024})
  ...
  (it "wire body for already-leading thinking is byte-identical (no churn for healthy messages)"
      (let [messages
            [{:role "user" :content "go"}
             {:role "assistant"
              :content [{:type "thinking" :thinking "ok" :thinking-signature "sig"}
                        {:type "text" :text "done"}]} {:role "user" :content "next"}]
  ...
  (it
    "wire body for [text, thinking] (model started with text, then a thinking trailer) demotes the trailing thinking"
    (let [messages
          [{:role "user" :content "hi"}
           {:role "assistant"
            :content [{:type "text" :text "hello"}
                      {:type "thinking" :thinking "wait" :thinking-signature "sig"}]}
            ...
            [{:role "user" :content "go"}
             {:role "assistant"
              :content [{:type "thinking" :thinking "" :thinking-signature ""}
                        {:type "text" :text "real body"}]} {:role "user" :content "next"}]
  ...
  (it "drops the empty text block but keeps a co-present signed thinking block (no collapse)"
      (let [messages
            [{:role "user" :content "go"}
             {:role "assistant"
              :content [{:type "thinking" :thinking "lead" :thinking-signature "sig-A"}
                        {:type "text" :text "body"}
                        {:type "thinking" :thinking "" :thinking-signature "sig-B"}]}
             {:role "user" :content "next"}]
