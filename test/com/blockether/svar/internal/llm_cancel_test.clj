...
(defdescribe
  cancel-watchdog-test
  (describe
    "fires when cancel-requested? becomes true, POST-headers (stream present)"
    (it "closes the stream and sets cancel-fired? once, WITHOUT interrupting the caller"
        ...
        (let [caller
              ...
              ; clear leftover interrupt
              t
              (start-cancel-watchdog caller cancel-req? stream-ref cancel-fired? alive?)]
          ...
          (loop [n 0]
            (when (and (not @cancel-fired?) (< n 100)) (Thread/sleep 20) (recur (inc n))))
          (reset! alive? false)
  ...
  (describe
    "fires when cancel-requested? becomes true, PRE-headers (no stream yet)"
    (it "interrupts the caller and sets cancel-fired?, with no stream to close"
        ...
        (let [caller
          ...
          (expect (true? @cancel-fired?)))))
  (describe
    "does NOT fire while cancel-requested? stays false"
    (it
      "leaves the stream open, caller uninterrupted, cancel-fired? false"
      (let [caller
            (Thread/currentThread)
            ...
            interrupted?
            (try (Thread/sleep 400) false (catch InterruptedException _ true))]
  ...
  (describe
    ...
    (it
      "does not fire after caller signals shutdown"
      (let [caller
            (Thread/currentThread)
