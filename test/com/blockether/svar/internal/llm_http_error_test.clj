...
(defdescribe truncate-error-body-test
             (describe "pure stringifier"
                       (it "returns nil for nil" (expect (nil? (truncate-error-body nil))))
                       (it "returns nil for blank string"
                           (expect (nil? (truncate-error-body "")))
                           (expect (nil? (truncate-error-body "   \n  "))))
                       (it "passes short strings through unchanged"
                           (expect (= "{\"error\":\"boom\"}"
                                      (truncate-error-body "{\"error\":\"boom\"}"))))
                       (it "truncates over-cap strings with a count suffix"
                           (let [body
                                 (apply str (repeat (+ MAX-CHARS 50) "x"))

                                 out
                                 (truncate-error-body body)]
                       ...
                       (with-retry (fn []
                                     (if (= 1 (swap! calls inc))
                                       (throw (ex-info "Exceptional status code: 529"
                                                       {:status 529}))
                                       :ok))
                                   {:max-retries 2 :initial-delay-ms 0})]
...
(defdescribe
  deliberate-stream-abort-test
  (describe
    "retryable-exception? rejects svar's own watchdog/cancel aborts"
    (it "treats a closed-stream idle timeout as non-retryable"
        (expect (false? (boolean (retryable-exception?
                                   (ex-info
                                     "Stream idle timeout (180000ms with no bytes): Stream closed"
        ...
        (expect
          (false?
            (boolean
              (retryable-exception?
                (ex-info
                  "Stream semantic timeout (240000ms without model/progress event): Stream closed"
                  {:type :svar.core/stream-semantic-timeout :stream? true}))))))
    (it "treats a caller cancellation as non-retryable"
        (expect (false? (boolean (retryable-exception? (ex-info "Stream cancelled: Stream closed"
...
(defdescribe
  http-error-ex-data-preserves-body-test
  (describe
    "non-stream chat-completion"
    (it "attaches the upstream response body to :svar.core/http-error ex-data"
        (let [anthropic-error-body
              (str "{\"type\":\"error\"," "\"error\":{\"type\":\"invalid_request_error\","
                   "\"message\":\"messages.1.content.1: Invalid `signature` in `thinking` block\"},"
                   "\"request_id\":\"req_011CarodswSWgEdPfFaJakLs\"}")

              captured
              (with-redefs-fn {#'com.blockether.svar.internal.llm/http-post!
                               (fn [_ _ _ _]
                                 (throw (ex-info "Exceptional status code: 400"
                                                 {:status 400 :body anthropic-error-body})))}
                (fn []
                  (try (sut/chat-completion [{:role "user" :content "hi"}]
                                            "claude-opus-4-7" "sk-fake"
                                            "https://api.anthropic.com/v1"
                                            {:api-style :anthropic :timeout-ms 1000 :max-retries 1})
  ...
  (describe
                ...
                {#'com.blockether.svar.internal.llm/http-post-stream!
                 (fn [_url _body _headers _timeout-ms _ttft-ms _idle-ms _delta-fn _on-delta]
                   (if (= 1 (swap! calls inc))
                     ...
                     {:content "ok" :reasoning nil :http-response {:status 200 :streaming? true}}))}
                (fn []
                  (sut/chat-completion [{:role "user" :content "hi"}]
                                       "glm-5.1" "sk-fake"
                                       "https://example.test/v1" {:on-chunk (constantly nil)
                                                                  :max-retries 2
                                                                  :initial-delay-ms 0})))]
    ...
    (it
      "retries a pre-response 'received no bytes' drop (proxy/tunnel closed before responding, NO :stream?)"
    ...
    (it "does not retry after reasoning started"
        (let [calls
              ...
              captured
              (with-redefs-fn {#'com.blockether.svar.internal.llm/http-post-stream!
                               (fn [& _]
                                 (swap! calls inc)
                                 (throw (ex-info "Stream connection error: closed"
                                                 {:type :svar.core/http-error
    ...
    (it "preserves partial reasoning on final stream failure"
        (let [captured (with-redefs-fn {#'com.blockether.svar.internal.llm/http-post-stream!
                                        (fn [& _]
                                          (throw (ex-info "Stream connection error: closed"
                                                          {:type :svar.core/http-error
                                                           :stream? true
...
(defdescribe
  transient-network-error-test
  (describe
    "transient-network-error? cause-chain matching"
    (it "matches EADDRNOTAVAIL (Can't assign requested address)"
        (expect (transient-network-error? (java.net.BindException.
                                            "Can't assign requested address"))))
    (it "matches when wrapped several layers deep"
        (expect (transient-network-error? (ex-info "Stream connection error"
                                                   {:type :svar.core/http-error}
    ...
    (it "matches no route to host / host unreachable / connect timeout"
        (expect (transient-network-error? (java.net.NoRouteToHostException. "No route to host")))
        (expect (transient-network-error? (java.net.ConnectException. "Connection timed out")))
        (expect (transient-network-error? (java.net.UnknownHostException. "api.anthropic.com"))))
    (it "does NOT match connection refused (down service, not a blip)"
        (expect (not (transient-network-error? (java.net.ConnectException. "Connection refused")))))
    (it "does NOT match unrelated errors"
        (expect (not (transient-network-error? (ex-info "Exceptional status code: 400"
                                                        {:status 400}))))
  ...
  (describe "retryable-exception? folds transient network errors in"
            (it "flags EADDRNOTAVAIL as retryable"
                (expect (retryable-exception? (ex-info "Can't assign requested address"
                                                       {:type :svar.core/http-error}
...
(defdescribe
  ...
  (describe
    "connection-error? cause-chain matching"
    ...
    (it "matches when wrapped several layers deep"
        (expect (connection-error? (ex-info "boom"
                                            {}
                                            (java.io.IOException. "io"
                                                                  (java.net.ConnectException.
                                                                    "Connection refused"))))))
    (it "does NOT match plain HTTP-status / non-network errors"
        (expect (not (connection-error? (ex-info "Exceptional status code: 400" {:status 400}))))
  ...
  (describe "connection-error->ex-info wrapping"
            (it "names the host, keeps :url + :connection-error?, preserves cause"
                (let [cause
                      ...
                      ex
                      (connection-error->ex-info cause "https://api.anthropic.com/v1/messages")]
  ...
  (describe
    "streaming chat-completion surfaces connect failures without retry"
    (it "does not retry a wrapped connect-phase failure and keeps the nice message"
        (let [calls
              ...
              (with-redefs-fn {#'com.blockether.svar.internal.llm/http-post-stream!
                               (fn [url & _]
                                 (swap! calls inc)
                                 (throw (connection-error->ex-info (java.net.ConnectException.
                                                                     "Connection refused")
                                                                   url)))}
                (fn []
                  (try (sut/chat-completion [{:role "user" :content "hi"}]
...
(defdescribe
  healthy-host-connect-blip-test
  (describe "message-less ConnectException gated by prior host success"
            (it "is NON-retryable when the host has NEVER connected (fail fast)"
                ...
                (let [ex (connection-error->ex-info (java.net.ConnectException.) ; message-less (JDK 25 shape)
