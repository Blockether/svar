...
(defn- items-of [stateless? type] (filterv #(= type (:type %)) (input-of stateless?)))

(defdescribe stateful-replay-is-unchanged-test
             (it "by default still replays the server-minted ids — public OpenAI wants them"
                 (let [reasoning
                       (first (items-of false "reasoning"))

                       fn-call
                       (first (items-of false "function_call"))]
...
(defdescribe
  stateless-replay-drops-server-ids-test
  (it "drops the reasoning id + encrypted_content and the function_call id"
      (let [reasoning
            (first (items-of true "reasoning"))

            fn-call
            (first (items-of true "function_call"))]
...
(defdescribe
  provider-opt-propagates-test
  (it
    "carries a provider's :stateless-items? into the per-call opts"
    (let [provider
          {:id :gw :api-key "k" :base-url "https://gw/v1" :api-style :openai-compatible-responses}]
...
(defdescribe
  item-affinity-error-test
  (it "recognises the Azure cross-resource item rejection"
      (expect
        (failure/item-affinity-error?
          (ex-info
            "Item 'rs_A' of type 'reasoning' was created under a different Azure OpenAI resource"
            {:status 400})))
...
(defdescribe
  ...
  (it "self-heals only before any visible output"
      (let [affinity (fn [data]
        ...
        (expect (not (failure/retry-without-server-item-ids? (ex-info "Overloaded"
                                                                      {:status 529}))))))
  (it "remembers the host that rejected replayed item ids"
      (let [url "https://stateless-test.example.invalid/v1"]
        (try (expect (not (failure/stateless-items-host? url)))
             ...
             (finally (swap! failure/stateless-item-hosts* disj
                        "stateless-test.example.invalid"))))))
