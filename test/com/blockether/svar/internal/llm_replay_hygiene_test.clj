...
(defn- responses-reasoning-count
  [target extra]
...
(defn- anthropic-has-assistant?
  [target extra]
  (boolean (some #(= "assistant"
                     (some-> (:role %)
                             name))
...
(defdescribe stamp-assistant-model-test
             (it "tags the canonical assistant message with the producing model"
                 (expect (= "m-A"
                            (get-in (stamp {:assistant-message {:role "assistant" :content []}}
                                           "m-A")
...
(defdescribe chat-wire-strips-model-stamp-test
             (it "drops the :model stamp from assistant messages in the OpenAI chat body"
                 (let [msgs
                       ...
                       body
                       (build-chat msgs "devstral-latest")]
...
(defdescribe
  cross-model-reasoning-guard-test
...
(defdescribe
  ...
  (it "injects a result for a tool_use dangling at the end of the array"
      (let [out (sanitize [{:role "user" :content "go"}
                           {:role "assistant"
                            :content [{:type "tool_use" :id "c1" :name "cat" :input {}}]}]
                          "m")]
  ...
  (it "merges a missing result into the partial results turn (Anthropic single-turn rule)"
      (let [out (sanitize [{:role "assistant"
                            :content [{:type "tool_use" :id "c1"} {:type "tool_use" :id "c2"}]}
                           {:role "user"
...
(defdescribe
  ...
  (it "rewrites a tool_use id and its tool_result id to the SAME normalized value"
      (let [out
            (sanitize [{:role "assistant" :content [{:type "tool_use" :id "fc|x!" :name "cat"}]}
                       {:role "user"
                        :content [{:type "tool_result" :tool_use_id "fc|x!" :content "ok"}]}]
                      "m")

            use-id
            (get-in out [0 :content 0 :id])

            res-id
            (get-in out [1 :content 0 :tool_use_id])]
  ...
  (it "leaves already-valid ids untouched"
      (let [msgs [{:role "assistant" :content [{:type "tool_use" :id "call_ok-1"}]}
                  {:role "user"
                   :content [{:type "tool_result" :tool_use_id "call_ok-1" :content "x"}]}]]
        (expect (= msgs (sanitize msgs "m"))))))

(defdescribe
  anthropic-wire-is-error-test
...
(defdescribe
  ...
  (it "extracts the composite call_id|item_id"
      (expect (= "call_abc|fc_xyz"
                 (responses-id {"call_id" "call_abc" "id" "fc_xyz" "name" "cat"})))
  ...
  (it "over-long composite halves stay <=64 AND keep the fc_ prefix (HTTP-400 regression)"
      ...
      (let [long-item
            (str "itm_" (apply str (repeat 90 "a")))

            out
            (normalize-id3 (str "call_abc|" long-item) true)

            [c i]
            (clojure.string/split out #"\|" 2)]
  ...
  (it "Responses re-emits function_call with BOTH id (fc_) and call_id, output keyed on call_id"
      (let [in
            (:input (build-responses
                      [{:role "assistant"
                        :content
                        [{:type "tool_use" :id "call_abc|fc_xyz" :name "cat" :input {"path" "x"}}]}
                       {:role "user"
                        :content
                        [{:type "tool_result" :tool_use_id "call_abc|fc_xyz" :content "ok"}]}]
                      "gpt-5"
                      {}))

            fc
            (first (filter #(= "function_call" (:type %)) in))

            fo
            (first (filter #(= "function_call_output" (:type %)) in))]
  ...
  (it "Anthropic replay of a composite id collapses it consistently (call↔result still match)"
      (let [blocks
            (->> (:messages
                   (build-anthropic
                     [{:role "assistant"
                       :content [{:type "tool_use" :id "call_abc|fc_xyz" :name "cat" :input {}}]}
                      {:role "user"
                       :content
                       [{:type "tool_result" :tool_use_id "call_abc|fc_xyz" :content "ok"}]}]
                     "claude-x"
            ...
            tu
            (first (filter #(= "tool_use" (:type %)) blocks))

            tr
            (first (filter #(= "tool_result" (:type %)) blocks))]
...
(defdescribe
  chat-completions-composite-id-test
  (it "collapses a replayed Responses composite id, call and result still paired"
      (let [composite
            (responses-id {"call_id" (str "call_" (apply str (repeat 24 \a)))
            ...
            call
            (first (mapcat :tool_calls (:messages body)))

            result
            (first (filter #(= "tool" (:role %)) (:messages body)))]
        ...
        (expect (re-matches wire-tool-id-re (str (:tool_call_id result)))
                (str "tool_call_id must match the chat wire form, got "
  ...
  (it "normalizes an orphan tool_result id the shared id map never saw"
      ...
      (let [body
            (build-chat [{:role "user"
                          :content
                          [{:type "tool_result" :tool_use_id "call_orphan|fc_zzz" :content "ok"}]}]
                        "gpt-5.6-sol")
...
(defdescribe
  chat-wire-composite-id-both-paths-test
  (it
    "posts a collapsed, paired tool id on the non-streaming and the streaming path"
    (let [composite
          (responses-id {"call_id" (str "call_" (apply str (repeat 24 \a)))
                         "id" (str "fc_" (apply str (repeat 60 \b)))
                         "name" "cat"})
          ...
          envelope
          {:parsed {"choices" [{"message" {"content" "ok"} "finish_reason" "stop"}]}
           :raw-body "{}"
      ...
      (with-redefs-fn {#'sut/http-post-stream! (fn [_url body _headers _timeout-ms _ttft _idle
                                                    _delta-fn _on-delta]
...
(defdescribe
  responses-reasoning-id-ceiling-test
  ...
  (it "clamps a >64-char reasoning id to <=64 and the wire contract"
      (let [convo
            [{:role "user" :content "go"}
             {:role "assistant"
              :content
              [{:type "thinking" :thinking "t" :thinking-signature sig-long-id :redacted? false}
               {:type "text" :text "ok"}]} {:role "user" :content "again"}]

            input
            (:input (build-responses convo "gpt-5" {}))

            r
            (first (filter #(= "reasoning" (:type %)) input))]
  ...
  (it "drops a decoded reasoning item whose id lacks the required rs_ prefix"
      (let [convo
            [{:role "user" :content "go"}
             {:role "assistant"
              :content
              [{:type "thinking" :thinking "t" :thinking-signature sig-non-rs-id :redacted? false}
               {:type "text" :text "ok"}]} {:role "user" :content "again"}]
...
(defdescribe persisted-wire-thinking-replay-test
             (it "normalizes Vis JSON-restored thinking without leaking canonical metadata"
                 (let [body
                       (build-anthropic [{:role "assistant"
                   ...
                   (expect (= {:type "thinking" :thinking "reason" :signature "sig"} block))
             ...
             (it "normalizes persisted redacted thinking"
                 (let [body (build-anthropic [{:role "assistant"
                                               :content [{"type" "thinking"
