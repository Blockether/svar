...
(defn- mock-chat-completion
  ...
  (fn [messages model api-key url retry-opts]
    (swap! calls-atom conj
      {:messages messages :model model :api-key api-key :url url :retry-opts retry-opts})
...
(defn- system-msgs [msgs] (filterv #(= "system" (:role %)) msgs))

(defn- user-msgs [msgs] (filterv #(= "user" (:role %)) msgs))

(defn- text-blocks
  "Returns the vec of text blocks for a message, normalizing string-content
   to a single-element vec for uniform inspection."
  [{:keys [content]}]
  (cond (string? content) [{:type "text" :text content}]
...
(defdescribe
  tail-pointer-default-on-test
  (describe
    "default behavior (no :schema-tail-pointer? key)"
        ...
        (let [calls (atom [])]
          (with-redefs [llm/chat-completion (mock-chat-completion {} calls)]
            (svar/ask! (test-router)
                       {:spec answer-spec
                        :messages [(svar/system "You are helpful.") (svar/user "Reply with ok.")]}))
    ...
    (it "places the pointer as the LAST text block of the last user message"
        (let [calls (atom [])]
          (with-redefs [llm/chat-completion (mock-chat-completion {} calls)]
            (svar/ask! (test-router)
                       {:spec answer-spec
                        :messages [(svar/user "first thing") (svar/user "second thing")]}))
          (let [msgs (:messages (first @calls))
            ...
            (expect (re-find #"schema in the system message" (joined-text last-u)))
            (expect (not (re-find #"schema in the system message" (joined-text (first users)))))
            ;; Pointer is the LAST block in the last-user content, not
            ;; somewhere in the middle.
            (expect (re-find #"schema in the system message" (:text (last blocks)))))))))
            ...
            (it "treats nil/missing as ON (only literal `false` opts out)"
                (let [calls (atom [])]
                  ...
                  (let [msgs (:messages (first @calls))
                    ...
                    (expect (re-find #"schema in the system message" (joined-text last-user))))))))
...
(defdescribe
  tail-pointer-multimodal-preserved-test
  (describe
        ...
        (let [calls
          ...
          (with-redefs [llm/chat-completion (mock-chat-completion {} calls)]
            (svar/ask! (test-router)
                       {:spec answer-spec
                        :messages [{:role "user"
                                    :content [{:type "text" :text "describe this"} img-block]}]}))
          (let [msgs
            ...
            (expect (re-find #"schema in the system message" (:text (last content))))
...
(defdescribe
  tail-pointer-past-cache-breakpoint-test
  (describe
    "with :cache-system? true on Anthropic api-style"
    (it
      "marks the system message cached and leaves the user pointer uncached"
      (let [calls
            (atom [])

            anthropic-router
            (svar/make-router [{:id :anth
                                :api-key "sk-test"
                                :base-url "https://example.invalid/v1"
                                :api-style :anthropic
        ...
        (with-redefs [llm/chat-completion (mock-chat-completion {} calls)]
          (svar/ask! anthropic-router
                     {:spec answer-spec
                      :messages [(svar/system "rules") (svar/user "Reply with ok.")]
                      :cache-system? true}))
        (let [msgs
              (:messages (first @calls))

              sys
              (first (system-msgs msgs))
          ...
          (let [pointer-block (->> user-blocks
                                   (filter #(re-find #"schema in the system message" (:text %)))
                                   first)]
