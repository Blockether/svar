...
(defdescribe
  format-retry-recovers-on-second-attempt-test
  (describe
    ":format-retries 2 with first-response-prose, second-response-valid"
    (it
      "succeeds and exposes :format-attempts with the failed attempt recorded"
      (let [calls
            (atom [])

            responses
            [{:content "Looking at what I have so far, I need to think..."}
             {:content "{\"answer\":\"ok\"}"}]]
        ...
        (with-redefs [llm/chat-completion (mock-chat-completion responses calls)]
          (let [result (svar/ask! (test-router)
            ...
            (let [[fail-att succ-att] (:format-attempts result)]
              ...
              ;; not truncated.
              (expect (= "Looking at what I have so far, I need to think..." (:content fail-att)))
...
(defdescribe
  format-retry-exhausted-throws-with-envelope-test
  (describe
    ":format-retries 1 with both responses prose"
    (it
      "throws after 2 attempts (initial + 1 retry) with full forensic envelope"
      (let [calls
            (atom [])

            ;; A long prose blob to verify NO truncation happens.
            long-prose
            (apply str "Looking at what I have so far, " (repeat 200 "blah blah "))

            responses
            [{:content long-prose} {:content long-prose}]]
             ...
             (describe ":format-retries 0 (default) on first prose response"
                       (it "throws after a single attempt with full envelope"
                           (let [calls
                                 ...
                                 responses
                                 [{:content "Looking at what I have so far"}
                                  ;; never reached
                                  {:content "{\"answer\":\"ok\"}"}]]

                             (with-redefs [llm/chat-completion (mock-chat-completion responses
                                                                                     calls)]
                               (let [thrown (try (svar/ask! (test-router)
                                                            {:spec answer-spec
...
(defdescribe
  format-retry-only-fires-on-listed-types-test
            ...
            (it "does not retry empty-content by default"
                (let [calls
                      ...
                      [{:content nil
                        ...
                        :http-response {:status 200 :stream-finalization {:finish-reason "stop"}}}
                       {:content "{\"answer\":\"ok\"}"}]]
...
(defdescribe
  json-object-mode-injects-response-format-test
  ...
  (describe ":json-object-mode? auto-from-model-metadata"
            (it "GLM model metadata flag propagates without explicit caller opt"
                ...
                (let [calls
                  ...
                  (with-redefs [llm/chat-completion (mock-chat-completion responses calls)]
                    (svar/ask! r {:spec answer-spec :messages [(svar/user "Reply glm")]})
                    (let [extra-body (get-in (first @calls) [:retry-opts :extra-body])]
                      (expect (= {:type "json_object"} (:response_format extra-body))))))))
  (describe "caller :extra-body :response_format always wins"
            (it "explicit caller-provided response_format is preserved verbatim"
                (let [calls
                      ...
                      custom-rf
                      {:type "json_schema" :json_schema {:name "custom"}}]
...
(defdescribe
  on-format-error-fallback-provider-test
  (describe
    ":on-format-error :fallback-provider with two providers, first emits prose"
    (it
      "falls back to second provider after first's format error"
      (let [calls
            (atom [])
        ...
        (with-redefs [llm/chat-completion (mock-chat-completion-by-model
  ...
  (describe
    ":on-format-error :fallback-provider with all providers failing"
    (it
      "throws the LAST format error verbatim with envelope + routing trace"
      (let [calls
            (atom [])
        ...
        (with-redefs [llm/chat-completion (mock-chat-completion-by-model
          ...
          (let [thrown (try (svar/ask! r
            ...
            (let [data (ex-data thrown)]
              ...
              (expect (= 1
                         (count (filter #(= :llm.routing/format-fallback (:event/type %))
                                        (:routed/trace data)))))
                ...
                (let [calls
                  ...
                  (with-redefs [llm/chat-completion
                                (mock-chat-completion-by-model
                                  ...
                                  calls)]
                    (let [thrown (try
                                   (svar/ask! r {:spec answer-spec :messages [(svar/user "Reply")]})
                                   ::no-throw
                                   (catch clojure.lang.ExceptionInfo e e))]
                      (expect (not= ::no-throw thrown))
             ...
             (describe
               ...
               (it "skips retries when streaming — first failure throws immediately"
                   (let [calls
                         ...
                         responses
                         [{:content "Looking at things"} {:content "{\"answer\":\"would-be\"}"}]]
