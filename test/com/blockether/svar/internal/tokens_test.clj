...
(defdescribe context-limit-test
             ...
             (describe
               "known models"
               (it "returns 128000 for gpt-4o" (expect (= 128000 (sut/context-limit "gpt-4o"))))
               ...
               (it "keeps provider-scoped GPT-6 Astra contexts exact"
                   (expect (= 1050000 (sut/provider-model-context :openai "gpt-6-astra")))
                   (expect (= 272000 (sut/provider-model-context :openai-codex "gpt-6-astra")))
                   (expect (= 922000 (sut/provider-model-context :github-copilot "gpt-6-astra"))))
...
(defdescribe count-tokens-test
             ...
             (it "counts tokens for simple text"
                 (let [tokens (sut/count-tokens "gpt-4o" "Hello, world!")]
                   ...
                   (expect (< tokens 10))))
             (it
               "returns more tokens for longer text"
               (let [short-tokens
                     (sut/count-tokens "gpt-4o" "Hi")
...
(defdescribe
  ...
  (it "calculates total as sum of input and output"
      (let [cost (sut/estimate-cost "gpt-4o" 1000000 1000000)]
        (expect (= (:total-cost cost) (+ (:input-cost cost) (:output-cost cost))))))
  ...
  (it "tracks public OpenAI standard cached pricing"
      (expect (= {:input 5.00
                 ...
                 (select-keys (:pricing (sut/estimate-cost "gpt-5.5" 1 1))
                              [:input :cached-input :output :input-over-272k :cached-input-over-272k
                               :output-over-272k])))
      (expect (= {:input 2.50
                  :cached-input 0.25
                  :output 15.00
                  :input-over-272k 5.00
                  :cached-input-over-272k 0.50
  ...
  (it "applies OpenAI long-context pricing to uncached cached and output components"
      (let [cost (sut/estimate-cost "gpt-5.5"
                                    300000
                                    1000
                                    {"gpt-5.5" {:input 5.00
                                                :cached-input 0.50
  ...
  (it
    ...
    (expect (= {:input 1.25
               ...
               (select-keys (:pricing (sut/estimate-cost "gemini-2.5-pro" 1 1))
                            [:input :cached-input :output :input-over-200k :cached-input-over-200k
                             :output-over-200k]))))
  (it "separates uncached input, cached input, output, and total cost"
      (let [pricing
            {"gpt-4o" {:input 2.00 :cached-input 0.50 :output 10.00}}

            cost
            (sut/estimate-cost "gpt-4o" 1000 500 pricing {:cached-tokens 400})]
  ...
  (it
    "prices Anthropic cache writes separately (Phase A: input-tokens is TOTAL inclusive)"
    ...
    (let [pricing
          ...
          cost
          (sut/estimate-cost "claude-sonnet-4-6"
                             1000
                             50
                             pricing
                             {:cached-tokens 100 :cache-creation-tokens 200})]
...
(defdescribe
  ...
  (it "returns token counts and cost"
      (let [messages
            ...
            result
            (sut/count-and-estimate "gpt-4o" messages output)]

        (expect (pos? (:input-tokens result)))
        (expect (pos? (:output-tokens result)))
        (expect (= (:total-tokens result) (+ (:input-tokens result) (:output-tokens result))))
        (expect (map? (:cost result)))
        (expect (pos? (get-in result [:cost :total-cost])))))
  (it
    "passes cached and cache-creation usage into cost breakdown (Phase A canonical shape)"
    (let [messages
          [{:role "system" :content [{:type "text" :text "stable" :svar/cache true}]}
           {:role "user" :content "Hello!"}]
          ...
          pricing
          {"gpt-4o" {:input 2.00 :cached-input 0.50 :cache-write-5m 2.50 :output 10.00}}

          result
          (sut/count-and-estimate "gpt-4o" messages "ok" {:api-usage api-usage :pricing pricing})]
...
(defdescribe format-cost-test
             ...
             (it "formats regular cost" (expect (= "$0.0025" (sut/format-cost 0.0025))))
             (it "formats very small cost" (expect (= "<$0.0001" (sut/format-cost 0.00001))))
             (it "formats zero cost" (expect (= "<$0.0001" (sut/format-cost 0)))))
             ...
             (describe "within limit"
                       (it "returns text unchanged when within limit"
                           (let [text
                                 ...
                                 result
                                 (sut/truncate-text "gpt-4o" text 100)]

                             (expect (= text result)))))
             (describe "exceeds limit"
                       (it "truncates text when exceeds limit"
                           (let [text
                                 (apply str (repeat 1000 "word "))

                                 result
                                 (sut/truncate-text "gpt-4o" text 10)]
...
(defdescribe check-context-limit-test
             ...
             (describe "within limit"
                       (it "returns ok for small messages"
                           (let [messages
                                 ...
                                 result
                                 (sut/check-context-limit "gpt-4o" messages)]
             ...
             (describe "exceeds limit"
                       (it "returns not ok for huge messages"
                           (let [huge-content
                                 (apply str (repeat 100000 "word "))

                                 messages
                                 [{:role "user" :content huge-content}]

                                 result
                                 (sut/check-context-limit "gpt-4" messages)]
