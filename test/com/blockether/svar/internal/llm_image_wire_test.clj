...
(defdescribe
  image-url-block->anthropic-test
  (describe "data URIs"
...
(defdescribe
  anthropic-request-body-image-test
  (it "emits native image blocks for a multimodal user message"
      (let [body
            (build-anthropic [(sut/user "what is on this screenshot?" (sut/image b64 "image/png"))]
                             "claude-test"
                             {})
        ...
        (expect (= {:type "base64" :media_type "image/png" :data b64} (:source (first blocks))))
