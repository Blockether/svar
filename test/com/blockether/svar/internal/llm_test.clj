...
(def test-providers
  [{:id :provider-a
    ...
    :models
    [{:name "tiny" :intelligence :low :speed :fast :cost :low :capabilities #{:chat}}
     {:name "medium"
      :intelligence :medium
      :speed :medium
      :cost :medium
...
(defdescribe single-prefer-test
             ...
             (describe "prefer :intelligence"
                       (it "selects the most intelligent model across the whole fleet"
                           ...
                           (expect (= "genius"
                                      (selected-model (make-router) {:prefer :intelligence})))))
...
(defdescribe
  ...
  (describe "[:intelligence :cost]"
            (it "smartest across the fleet, cheapest as tiebreaker within tier"
                ...
                (expect (= "genius"
                           (selected-model (make-router) {:prefer [:intelligence :cost]})))))
  (describe "[:speed :intelligence]"
            (it "fastest first, smartest as tiebreaker"
                (expect (= "tiny"
                           (selected-model (make-router) {:prefer [:speed :intelligence]})))))
  (describe
    "single-element vector matches keyword"
    (it "[:cost] same as :cost"
        (let [r (make-router)]
          (expect (= (selected-model r {:prefer :cost}) (selected-model r {:prefer [:cost]})))))
    ...
    (it "[:speed] same as :speed"
        (let [r (make-router)]
          (expect (= (selected-model r {:prefer :speed}) (selected-model r {:prefer [:speed]})))))))

(defdescribe
  capabilities-filter-test
  (describe
    "requiring :vision"
    (it "excludes non-vision models"
        (expect (not= "tiny"
                      (selected-model (make-router) {:prefer :cost :capabilities #{:vision}}))))
    ...
    (it "smartest vision-capable model across the fleet"
        ...
        (expect (= "genius"
                   (selected-model (make-router)
                                   {:prefer :intelligence :capabilities #{:vision}})))))
  (describe "vector prefer with capabilities"
            (it "[:cost :speed] with vision"
                (expect (= "medium"
                           (selected-model (make-router)
                                           {:prefer [:cost :speed] :capabilities #{:vision}}))))))
...
(defdescribe
  ...
  (it "exclude works with vector preferences"
      (expect (not= "tiny"
                    (selected-model (make-router)
                                    {:prefer [:cost :speed] :exclude-model "tiny"})))))

(defdescribe
  edge-cases-test
  (it "no preference returns a model" (expect (some? (selected-model (make-router) {}))))
  (it "empty vector returns a model" (expect (some? (selected-model (make-router) {:prefer []}))))
  (it "nil prefer returns a model" (expect (some? (selected-model (make-router) {:prefer nil})))))

(defdescribe
  provider-models-list-test
  (it "filters OpenAI Codex /models below GPT-5.3"
      (let [router (svar/make-router
                     [{:id :openai-codex :api-key "sk-test" :models [{:name "gpt-5.5"}]}])]
        ...
        (with-redefs-fn {#'sut/http-get! (fn [_url _api-key & _opts]
                                           {"data" [{"id" "gpt-4o"} {"id" "gpt-5"}
                                                    {"id" "gpt-5.1-codex"} {"id" "gpt-5.3-codex"}
                                                    {"id" "gpt-5.4"} {"id" "gpt-5.5"}]})}
          (fn []
            (expect (= ["gpt-5.3-codex" "gpt-5.4" "gpt-5.5"] (mapv :id (svar/models! router))))))))
  (it "filters GitHub Copilot /models below GPT-5.3 while keeping non-GPT families"
      (let [router (svar/make-router
                     [{:id :github-copilot :api-key "sk-test" :models [{:name "gpt-5.4"}]}])]
        (sut/clear-models-cache!)
        (with-redefs-fn {#'sut/http-get! (fn [_url _api-key & _opts]
...
(defdescribe
  transparent-openai-responses-routing-test
  (describe
    "ask! transparency"
    (it
      "ask! uses provider-level responses-path, headers, pricing, context, and dynamic verbosity"
      (let [calls
            (atom [])
            ...
            [provider model]
            (sut/select-provider router {:strategy :root})]
        ...
        (expect (= {:input 5.00
                   ...
                   (select-keys (:pricing model)
                                [:input :cached-input :output :input-over-272k
                                 :cached-input-over-272k :output-over-272k])))
        (expect (= 272000 (:context model)))
        (with-redefs-fn {#'sut/http-post-stream!
                         (fn [url body headers _timeout-ms _ttft-timeout-ms _idle-timeout-ms
                              _delta-fn on-delta]
                           (swap! calls conj
                             {:url url :body body :headers headers :on-delta on-delta})
                           {:content "{\"answer\":\"ok\"}"
                            :reasoning nil
                            ...
                            :http-response {:url url :streaming? true :status 200}})}
          (fn []
            (let [result
                  (svar/ask! router
                             {:spec answer-spec
                              :messages [(svar/system "Return JSON.") (svar/user "Reply ok")]
    ...
    (it
      "Copilot Responses requests reuse OpenAI reasoning and honor Copilot header overrides"
      (let [calls
            (atom [])
        ...
        (with-redefs-fn {#'sut/http-post-stream!
                         (fn [url body headers _timeout-ms _ttft-timeout-ms _idle-timeout-ms
                           ...
                           {:content "{\"answer\":\"ok\"}"
                            :reasoning nil
                            :api-usage {:input-tokens 10 :output-tokens 5 :total-tokens 15}
                            :http-response {:url url :streaming? true :status 200}})}
          (fn []
            (let [result
                  (svar/ask! router
              ...
              (expect (= ["reasoning.encrypted_content"] (:include body))))))))
    (it
      "chat-completion sends Anthropic OAuth tokens with Claude Code headers and system identity"
      (let [seen
            (atom nil)

            messages
            [(svar/system "Follow project rules.") (svar/user "hi")]]
        ...
        (let [seen
          ...
          (with-redefs-fn {#'sut/http-post! (fn [_url _body headers _timeout-ms]
            ...
            (fn []
              (sut/chat-completion messages
                                   "gpt-4o" "sk-test"
                                   "https://example.invalid/v1" {:provider-id :github-copilot
                                                                 :llm-headers {"User-Agent"
                                                                               "VisCopilot/0.1"}})
    ...
    (it "chat-completion marks Copilot requests with prior assistant turns as agent initiated"
        (let [seen
              ...
              messages
              [(svar/user "first") {:role "assistant" :content "previous"} (svar/user "continue")]]
          ...
          (with-redefs-fn {#'sut/http-post-stream!
            ...
            (fn []
              (let [result
                    ...
                    {:keys [url body headers on-delta]}
                    (first @calls)]
...
(defdescribe
  ...
  (describe "responses-url"
            (it "rewrites chat-completions URLs and preserves existing responses URLs"
                ...
                (expect (= "https://chatgpt.com/backend-api/codex/responses"
                           (sut/responses-url "https://chatgpt.com/backend-api/codex"
                                              "/codex/responses")))
                (expect (= "https://chatgpt.com/backend-api/codex/responses"
                           (sut/responses-url "https://chatgpt.com/backend-api/codex/responses"
                                              "/codex/responses")))))
  (describe
    "non-stream response fallback"
    (it "responses transport extracts content + reasoning from terminal :output payload"
        (with-redefs-fn {#'sut/http-post!
                         (fn [_url _body _headers _timeout-ms]
                           {:parsed
                            {"output" [{"type" "reasoning" "summary" [{"text" "plan first"}]}
                                       {"type" "message"
  ...
  (describe
    "stream response fallback"
    (it
      "responses transport does not duplicate output_text.done after deltas"
      (let
        [stream
         (str
    ...
    (it
      ...
      (let
        ...
        (with-redefs [http/post (fn [_url _opts]
          ...
          (let [error (try (sut/openai-responses-completion
                             {:model "test-model"
                              :input [{:role "user" :content [{:type "input_text" :text "hi"}]}]}
                             {:api-key "sk-test"
                              :base-url "https://example.invalid/v1"
    ...
    (it
      "responses transport backfills reasoning from output_item.done summaries"
      (let
        [events
         (atom [])
    ...
    (it "responses transport preserves Pi-style raw reasoning item signature for replay"
        (let [raw-item
              ...
              stream
              (str "data: "
                   (json/write-json-str {:type "response.output_item.done" :item raw-item})
                   "\n\n"
                   "data: {\"type\":\"response.output_text.delta\",\"delta\":\"(def x 1)\"}\n\n"
                   "data: [DONE]\n\n")]
          ...
          (with-redefs [http/post (fn [_url _opts]
            ...
            (let [result (sut/openai-responses-completion
              ...
              (expect (= raw-item
                         (get-in result [:provider-state :reasoning-items 0 :raw-item])))))))
    (it
      "responses transport reconstructs streamed summary into raw reasoning item when done omits it"
      (let
        [stream
         (str
        ...
        (with-redefs [http/post (fn [_url _opts]
          ...
          (let [result (sut/openai-responses-completion
            ...
            (expect (= [{"type" "summary_text" "text" "plan first"}]
                       (get-in result
                               [:provider-state :reasoning-items 0 :raw-item "summary"])))))))
    (it
      "responses transport separates multiple streamed reasoning summary parts"
      (let
        [events
         (atom [])
    ...
    (it
      "responses transport stores encrypted-only reasoning only in provider-state"
      (let
        [events
         (atom [])
    ...
    (it "responses transport preserves whitespace-only reasoning deltas like content deltas"
        (let [events
              ...
              stream
              (str "data: {\"type\":\"response.output_text.delta\",\"delta\":\"pass\"}\n\n"
  ...
  (describe
    ...
    (it ":models-base :host hangs path off host root, not the /v1 chat base"
        (expect (= "http://localhost:1234/api/v0/models"
                   ((var-get #'sut/models-endpoint-url)
                     "http://localhost:1234/v1"
                     :host
                     "/api/v0/models"))))
    (it
      "respects host/port overrides"
      (expect
        (= "http://10.0.0.5:9000/api/v0/models"
...
(defdescribe
  routed-context-limit-flow-test
  ...
  (let [inject
        ...
        resolve-opts
        (var-get #'sut/resolve-opts)

        sut-router-default
        @(requiring-resolve 'com.blockether.svar.internal.router/DEFAULT_CONTEXT_LIMIT)]

    (describe "inject-routed-params forwards :context"
              (it "carries the model-map's :context into opts"
                  (expect (= 262144
                             (:context (inject {} {:id :lmstudio} {:name "m" :context 262144})))))
    ...
    (describe "resolve-opts context-limits"
              (it "prefers an explicit :context over the static catalog default"
                  (let [resolved (resolve-opts {}
                                               {:model "m" :provider-id :lmstudio :context 262144})]
             ...
             (let [inject
               ...
               (it "injects exact high/max bodies"
                   (doseq [effort ["high" "max"]]
                     (let [body (:extra-body (inject {:reasoning-effort effort} provider model))]
               ...
               (it "raw effort bypasses abstract translation when both are supplied"
                   (let [body (:extra-body
                                (inject {:reasoning :low :reasoning-effort "max"} provider model))]
                     (expect (= "max" (:reasoning_effort body)))
                     (expect (= {:type "enabled"} (:thinking body)))))
               (it "consumes both public reasoning controls before the wire primitive"
                   (let [opts (inject {:reasoning :deep :reasoning-effort "high"} provider model)]
...
(defdescribe
  ...
  (let [build
        (var-get #'sut/build-request-body)

        msg
        (fn [blocks]
          [{:role "assistant" :content blocks} {:role "user" :content "next"}])
    ...
    (it "Anthropic redacted block (encrypted data) is dropped from a foreign wire entirely"
        (let [body (build (msg [{:type "thinking"
                                 ...
                                 :redacted? true} {:type "text" :text "answer"}])
                          "glm-5")]
...
(defdescribe
  anthropic-stream-finish-reason-test
  ...
  (let [finish (var-get #'sut/stream-finish-reason)]
    (it "reads stop_reason from an Anthropic message_delta"
        (expect (= "end_turn"
                   (finish {"type" "message_delta"
                            "delta" {"stop_reason" "end_turn" "stop_sequence" nil}
                            "usage" {"output_tokens" 42}})))
        (expect (= "max_tokens"
                   (finish {"type" "message_delta" "delta" {"stop_reason" "max_tokens"}}))))
    (it "OpenAI chat shape still wins its own field"
        (expect (= "stop" (finish {"choices" [{"finish_reason" "stop"}]}))))))

(defdescribe
  stream-semantic-event-test
  ...
  (let [semantic?
        (var-get #'sut/stream-semantic-event?)

        extract
        (var-get #'sut/extract-stream-delta)
    ...
    (it "ignores transport and nonterminal lifecycle events"
        (expect (= {"ping" false
                   ...
                   (into {}
                         (map (fn [event]
                                [(get event "type") (classify event)]))
                         [{"type" "ping"} {"type" "heartbeat"}
    ...
    (it "still recognizes model output and terminal progress"
        (expect (= {"response.output_text.delta" true
...
(defdescribe
  ...
  (let [classify (var-get #'sut/empty-reply-anomaly-type)]
    ...
    (it
      "an unknown / missing finish reason (mid-stream truncation) stays a retryable empty-content blip"
