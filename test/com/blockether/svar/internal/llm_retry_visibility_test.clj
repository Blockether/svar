  ...
  (describe "budget"
            (it "keeps retrying an overload past the old 5-attempt ceiling"
                (let [attempts
                      ...
                      (#'llm/with-retry
                       (fn []
                         (if (< (long (swap! attempts inc)) 6) (throw (overloaded!)) :ok))
                       fast)]
  ...
  (describe
    ...
    (it "announces every wait as a routing event the surfaces already paint"
        (let [seen
          ...
          (#'llm/with-retry
           (fn []
             (if (< (long (swap! attempts inc)) 3) (throw (overloaded!)) :ok))
           (assoc fast
    ...
    (it "says nothing when the call succeeds"
        (let [seen (atom [])]
          (expect
            (= :ok (#'llm/with-retry (constantly :ok) (assoc fast :on-retry #(swap! seen conj %)))))
          (expect (empty? @seen))))))
...
(defdescribe
  streamed-overload-ladder-test
  (describe
    "one policy for both ladders"
    ...
    (it
      "classifies the streamed OpenAI overload as router-retryable"
      (let
        [e
         (ex-info
           "Provider stream failed (server_is_overloaded): Our servers are currently overloaded. Please try again later."
           {:type :svar.core/stream-failed
            :source :provider
            :stream? true
            :status 529
...
(defn- rate-limited!
  ...
  (ex-info
    ...
    {:type :svar.core/http-error :status 429 :headers {"retry-after" (str retry-after-secs)}}))
