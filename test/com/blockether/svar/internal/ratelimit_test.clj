...
(defdescribe
  ...
  (describe
    "openai / codex — relative duration resets"
    ...
    (it "parses compound durations (1h2m3s, 100ms)"
        (let [r (sut/parse :openai-compatible-chat {"x-ratelimit-reset-requests" "1h2m3s"} now)]
          (expect (= (+ now 3723000) (get-in r [:windows :requests :resets-at-ms]))))
        (let [r (sut/parse :openai-compatible-chat {"x-ratelimit-reset-tokens" "100ms"} now)]
  ...
  (describe "absent / malformed headers"
            ...
            (it "is case-insensitive on header names"
                (let [r
                      (sut/parse :anthropic {"Anthropic-RateLimit-Unified-Reset" "1700003600"} now)]
                  (expect (= 1700003600000 (:resets-at-ms r)))))))
