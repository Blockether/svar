...
(defdescribe
  ...
  (describe "strict JSON parsing"
            (it "parses simple object without warnings"
                (expect (= {:value {:a 1} :warnings []} (sut/parse-json "{\"a\": 1}"))))
  ...
  (describe "multi-object extraction"
            (it "extracts objects from text (returns vector of candidates)"
                (let [result (sut/parse-json "Response: {\"id\": 1} and {\"id\": 2}")]
                  (expect (or (map? (:value result)) (vector? (:value result))))))
  ...
  (describe "error handling"
            (it "throws IllegalArgumentException on nil input"
                (expect (throws? IllegalArgumentException #(sut/parse-json nil))))
            (it "throws IllegalArgumentException on empty string"
                (expect (throws? IllegalArgumentException #(sut/parse-json ""))))))
...
(defdescribe
  ...
  (describe
    ...
    (it "parses top-level EDN keyword key :code"
        (let [result (sut/parse-json "{:code [{:expr \"(def foo 42)\" :time-ms 100}]}")]
          (expect (= {:code [{:expr "(def foo 42)" :time-ms 100}]} (:value result)))))
    (it "parses multiple EDN keyword keys at top level"
        (let [result (sut/parse-json "{:code [{:expr \"x\" :time-ms 10}] :answer \"done\"}")]
          (expect (= {:code [{:expr "x" :time-ms 10}] :answer "done"} (:value result)))))
    (it "parses nested EDN keyword keys in array elements"
        (let [result
              (sut/parse-json
                "{:code [{:expr \"(+ 1 2)\" :time-ms 50} {:expr \"(def m {})\" :time-ms 100}]}")]
          (expect (= {:code [{:expr "(+ 1 2)" :time-ms 50} {:expr "(def m {})" :time-ms 100}]}
                     (:value result)))))
  ...
  (describe
    ...
    (it "parses mixed EDN keys + EDN keyword values"
        (let [result (sut/parse-json
                       "{:code [{:expr \"x\" :time-ms 10}] :next {:reasoning :deep}}")]
          (expect (= {:code [{:expr "x" :time-ms 10}] :next {:reasoning "deep"}} (:value result)))))
    ...
    (it "parses keyword values with hyphens and dots"
        (let [result (sut/parse-json "{:content-type :application/json :encoding :utf-8}")]
          (expect (= {:content-type "application/json" :encoding "utf-8"} (:value result)))))
    (it "parses keyword values with namespaces (slash-separated)"
        (let [result (sut/parse-json "{:type :spec.type/string :required :spec/required}")]
          (expect (= {:type "spec.type/string" :required "spec/required"} (:value result)))))
    ...
    (it "parses keyword value followed by another EDN key (implicit comma)"
        (let [result (sut/parse-json "{:status :ok :code 200 :msg \"success\"}")]
          (expect (= {:status "ok" :code 200 :msg "success"} (:value result)))))
    ...
    (it "parses keyword value followed by closing bracket in array"
        (let [result (sut/parse-json "{:items [{:state :pending} {:state :active}]}")]
          (expect (= {:items [{:state "pending"} {:state "active"}]} (:value result)))))
    ...
    (it "parses keyword value with numbers in name"
        (let [result (sut/parse-json "{:protocol :http2 :fallback :tls1.3}")]
          (expect (= {:protocol "http2" :fallback "tls1.3"} (:value result)))))
    (it "parses deeply nested keyword values at multiple levels"
        (let [result (sut/parse-json
                       "{:a {:state :running :b {:state :idle :c {:state :stopped}}}}")]
  ...
  (describe "incomplete / truncated inputs (lenient end)"
            ...
            (it "parses empty map" (expect (= {:value {} :warnings []} (sut/parse-json "{}"))))
            (it "parses empty array" (expect (= {:value [] :warnings []} (sut/parse-json "[]")))))
  (describe
    "mixed JSON + EDN (real production variants)"
    ...
    (it "handles EDN with Clojure-style map inside :expr string"
        ...
        (let
          [result
           (sut/parse-json
             "{:code [{:expr \"(list-dir \\\".\\\" {:glob \\\"**/*.clj\\\"})\" :time-ms 3000}]}")]
  ...
  (describe
    ...
    (it "parses nested EDN map inside array inside map"
        (let [result (sut/parse-json "{:results [{:meta {:tags [:important :urgent]} :score 95}]}")]
          (expect (= {:results [{:meta {:tags ["important" "urgent"]} :score 95}]}
                     (:value result)))))
    (it
      "parses deeply nested mixed keyword values and maps"
      (let
        [result
         (sut/parse-json
           "{:config {:mode :fast :limits {:max-depth 10 :timeout 5000} :flags [:verbose :debug]}}")]
        (expect (= {:config
                    {:mode "fast" :limits {:max-depth 10 :timeout 5000} :flags ["verbose" "debug"]}}
                   (:value result)))))
    (it "parses array of arrays with EDN keywords"
        (let [result (sut/parse-json "{:matrix [[{:val 1} {:val 2}] [{:val 3} {:val 4}]]}")]
          (expect (= {:matrix [[{:val 1} {:val 2}] [{:val 3} {:val 4}]]} (:value result)))))
    (it
      "parses real iteration-spec shape with :final nested map"
      (let
        [result
         (sut/parse-json
           "{:code [{:expr \"(+ 1 2)\" :time-ms 50}] :final {:answer \"The answer is 3\" :confidence :high :language :en}}")]
        (expect (= {:code [{:expr "(+ 1 2)" :time-ms 50}]
                    :final {:answer "The answer is 3" :confidence "high" :language "en"}}
                   (:value result)))))
    (it
      "parses multiple code blocks with nested forms as strings"
      (let
        [result
         (sut/parse-json
           "{:code [{:expr \"(defn greet [name] (str \\\"Hello \\\" name))\" :time-ms 200} {:expr \"(greet \\\"world\\\")\" :time-ms 50}]}")]
    ...
    (it "parses deeply truncated nested structure"
        (let
          [result
           (sut/parse-json
             "{:code [{:expr \"(+ 1 2)\" :time-ms 50}] :next {:reasoning :deep :context {:depth 3")]
          (expect (map? (:value result)))
          (expect (contains? (:value result) :code))
          (expect (contains? (:value result) :next))))
    (it
      "parses nested EDN inside clojure markdown fence"
      (let
        [result
         (sut/parse-json
        ...
        (expect (= {:code [{:expr "(map inc [1 2 3])" :time-ms 100}]
                   ...
                   (:value result)))))
    (it
      "parses nested EDN inside edn markdown fence"
      (let
        [result
         (sut/parse-json
  ...
  (describe
    ...
    (it
      "parses multi-turn conversation shape with nested arrays of maps"
      (let
        [result
         (sut/parse-json
    ...
    (it
      "parses map where values are heterogeneous: strings, numbers, booleans, keywords, nested maps, arrays"
      (let
        [result
         (sut/parse-json
    ...
    (it
      "parses array at top level containing nested EDN maps"
      (let
        [result
         (sut/parse-json
    ...
    (it
      "parses EDN with string values containing colons, braces, brackets"
      (let
        [result
         (sut/parse-json
           "{:expr \"(let [{:keys [a b]} m] (str a \\\":\\\" b))\" :note \"handles {:keys} destructuring [and arrays]\"}")]
        (expect (= "(let [{:keys [a b]} m] (str a \":\" b))" (:expr (:value result))))
        (expect (= "handles {:keys} destructuring [and arrays]" (:note (:value result))))))
    (it
      "parses massive iteration-spec shape: code + next + final all present"
      (let
        [result
         (sut/parse-json
    ...
    (it
      "parses truncated mid-string inside deeply nested structure"
      (let
        [result
         (sut/parse-json
        ...
        (expect (= "deep" (get-in (:value result) [:next :reasoning])))))
    (it
      "parses EDN fence with keyword values in arrays (vector of keywords)"
      (let
        [result
         (sut/parse-json
        ...
        (expect (= ["read" "write" "admin" "delete"] (get-in (:value result) [:roles 0 :grants])))
        (expect (= ["read"] (get-in (:value result) [:roles 1 :grants])))))
    (it
      "parses interleaved JSON + EDN keys in nested structure"
      (let
        [result
         (sut/parse-json
