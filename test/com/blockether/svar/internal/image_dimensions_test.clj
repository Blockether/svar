...
(defn- le24
  [^long n]
  [(bit-and n 0xff) (bit-and (bit-shift-right n 8) 0xff) (bit-and (bit-shift-right n 16) 0xff)])
...
(defn- bmp [w h] (bytes' (concat (ascii "BM") (repeat 16 0) (le32 w) (le32 h) [1 0 24 0])))
...
(defn- webp
  [fourcc payload]
  (bytes' (concat (ascii "RIFF")
                  (le32 (+ 12 (count payload)))
                  (ascii "WEBP")
                  (ascii fourcc)
                  (le32 (count payload))
                  payload)))
...
(defn- webp-lossless
  ...
  (let [wm
    ...
    (webp
      "VP8L"
      [0x2f (bit-and wm 0xff) (bit-or (bit-shift-right wm 8) (bit-and (bit-shift-left hm 6) 0xc0))
       (bit-and (bit-shift-right hm 2) 0xff) (bit-and (bit-shift-right hm 10) 0x0f) 0 0 0 0 0])))
...
(defdescribe
  ...
  (describe "recognised headers"
            (it "reads PNG IHDR" (expect (= [637 213] (dimensions (png 637 213)))))
            (it "reads JPEG SOFn after walking past APP0 and DQT"
                (expect (= [637 213] (dimensions (jpeg 637 213)))))
            (it "reads the GIF logical screen" (expect (= [637 213] (dimensions (gif 637 213)))))
  ...
  (describe
    "everything else is nil, never a throw"
    ...
    (it "rejects a PNG whose IHDR never arrives"
        (expect (nil? (dimensions (bytes' (concat [0x89]
                                                  (ascii "PNG\r\n")
                                                  [0x1a 0x0a]
                                                  (be32 13)
                                                  (ascii "junk")
    ...
    (it "rejects a RIFF container that is not WebP"
        (expect (nil? (dimensions
                        (bytes' (concat (ascii "RIFF") (le32 0) (ascii "WAVE") (repeat 16 0)))))))))

(defdescribe vision-token-estimate-test
             (describe
               "data: URLs are measured, not guessed"
               (it "prices a 637x213 PNG by its real size"
                   (let [estimate
                         #'sut/estimate-image-block-tokens

                         b64
                         (.encodeToString (Base64/getEncoder) (png 637 213))

                         block
                         {:type "image_url" :image_url {:url (str "data:image/png;base64," b64)}}]
               ...
               (it "falls back when the payload is unreadable"
                   (let [estimate
                         ...
                         b64
                         (.encodeToString (Base64/getEncoder) (bytes' (range 32)))]

                     (expect (= 765
                                (long (estimate {:type "image_url"
                                                 :image_url {:url (str "data:image/png;base64,"
                                                                       b64)}}))))))
