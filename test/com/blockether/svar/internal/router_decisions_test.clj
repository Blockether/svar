...
(defn- advance! [clock-atom ms] (swap! clock-atom + ms))
...
(defn- transient-error
  ...
  [status]
  (throw (ex-info (str "HTTP " status) {:type :svar.core/http-error :status status})))
...
(defn- stream-truncated-error
  ...
  ([content-len]
   (throw (ex-info
            ...
            (cond-> {:type :svar.core/stream-truncated :stream? true :content-acc-len content-len}
              (pos? (long content-len))
              (assoc :partial-content "x"))))))
...
(defdescribe
  ...
  (let [router
        (llm/make-router
          [{:id :prov-a :api-key "k" :base-url "http://a" :models [{:name "m-a1"} {:name "m-a2"}]}
           {:id :prov-b :api-key "k" :base-url "http://b" :models [{:name "m-b1"}]}])]
    ...
    (it "`:provider` + `:optimize` → `:force-provider` + `:prefer` from optimize"
        (let [{:keys [prefs]} (router/resolve-routing router {:provider :prov-a :optimize :speed})]
          (expect (= :prov-a (:force-provider prefs)))
          (expect (= :speed (:prefer prefs)))))
    (it "`:provider` + `:model` exact → both force-* set"
        (let [{:keys [prefs]} (router/resolve-routing router {:provider :prov-a :model "m-a2"})]
          (expect (= :prov-a (:force-provider prefs)))
          (expect (= "m-a2" (:force-model prefs)))))
    (it "`:provider` + unknown-provider throws :svar/routing-resolution-failed"
        (expect (throws? clojure.lang.ExceptionInfo
                         #(router/resolve-routing router {:provider :nonexistent :model "m-a1"}))))
    (it "`:provider` + model-not-in-provider throws"
        (expect (throws? clojure.lang.ExceptionInfo
                         #(router/resolve-routing router {:provider :prov-a :model "m-b1"})))) ;; m-b1 is in :prov-b
    (it "`:on-transient-error` overrides default :hybrid"
        (let [{:keys [error-strategy]} (router/resolve-routing router {:on-transient-error :fail})]
    ...
    (it "`:on-chunk` is threaded into prefs so live routing events surface"
        ...
        (let [cb (fn [_e])
              {:keys [prefs]} (router/resolve-routing router {:optimize :cost :on-chunk cb})]
...
(defdescribe
  provider-pin-honors-optimize-test
  ...
  (let [router (llm/make-router [{:id :plan
                                  :api-key "k"
                                  :base-url "http://p"
                                  :models [{:name "big" :cost :high} {:name "small" :cost :low}]}])]
    (it "provider + :optimize :cost picks the cheapest model, not the root"
        (let [{:keys [prefs]} (router/resolve-routing router {:provider :plan :optimize :cost})
...
(defdescribe
  prefer-providers-ordered-chain-test
  ...
  (let [router
        (llm/make-router
          ...
          [{:id :copilot
           ...
           {:id :zai :api-key "k" :base-url "http://z" :models [{:name "glm-turbo" :cost :low}]}])]
    (it "resolve-routing translates :prefer-providers → :provider-order + :prefer"
        (let [{:keys [prefs]} (router/resolve-routing router
                                                      {:prefer-providers [:zai :anthropic]
                                                       :optimize [:cost :speed]})]
    ...
    (it "selects the first preferred provider's cheapest model, ignoring priority"
        (let [{:keys [prefs]} (router/resolve-routing router
                                                      {:prefer-providers [:zai :anthropic :copilot]
                                                       :optimize :cost})
    ...
    (it "walks the chain in order on model-unsupported failures, smallest per plan"
        (let [calls (atom [])
          ...
          (expect (= [[:zai "glm-turbo"] [:anthropic "haiku"] [:anthropic "opus"]
                      [:copilot "cop-small"]]
                     @calls))
    ...
    (it "providers absent from the preference list are tried last"
        (let [{:keys [prefs]}
              (router/resolve-routing router {:prefer-providers [:anthropic] :optimize :cost})
              [prov _] (router/select-provider router prefs)]

          (expect (= :anthropic (:id prov)))))))

(defdescribe
  with-provider-fallback-happy-path-test
...
(defdescribe
  with-provider-fallback-transient-then-success-test
...
(defdescribe
  with-provider-fallback-stream-truncated-test
  (it
    "falls through to P2 on zero-content stream truncation"
    (let [[clock _]
          (mock-clock)
      ...
      (doseq [timeout-type [:svar.core/stream-ttft-timeout :svar.core/stream-idle-timeout
        ...
        (let [[clock _] (mock-clock)
              r (llm/make-router
                  [{:id :p1 :api-key "k" :base-url "http://p1" :models [{:name "m1"}]}
                   {:id :p2 :api-key "k" :base-url "http://p2" :models [{:name "m2"}]}]
                  {:clock clock :rate-limit {:same-provider-delays-ms [0 0] :fallback-after-ms 0}})
              calls (atom [])
              failure (ex-info
...
(defdescribe
  ...
  (it
    "retries same provider, then falls back with one event shape"
    (let [[clock _]
          (mock-clock)
  ...
  (it
    "`:respect-retry-after?` lets `Retry-After` header override the configured delay"
    (let [[clock _]
          (mock-clock)
          ...
          #(throw (ex-info "HTTP 429"
                           {:type :svar.core/http-error :status 429 :headers {"retry-after" "0"}}))
      ...
      (let [trace
            ...
            retries
            (filter #(= :llm.routing/provider-retry (:event/type %)) trace)]
  ...
  (it
    "falls back instead of sleeping less than the declared cooldown"
    (let [[clock _]
          (mock-clock)
      ...
      (expect (empty? (filter #(= :llm.routing/provider-retry (:event/type %))
                              (:routed/trace result))))))
  (it
    "`:respect-retry-after? false` ignores `Retry-After` header"
    (let [[clock _]
          (mock-clock)
          ...
          #(throw (ex-info
                    "HTTP 429"
                    {:type :svar.core/http-error :status 429 :headers {"retry-after" "9999"}}))
      ...
      (expect (= [:p1 :p1 :p1 :p2] @calls))
      (let [retries (filter #(= :llm.routing/provider-retry (:event/type %))
                            (:routed/trace result))]
        (expect (every? #(zero? (long (:delay-ms %))) retries)))))
  ...
  (it "`:fallback-after-ms` caps the same-provider phase — delays clamp, no padding"
      ...
      (let [clock-atom
        ...
        (let [retries
              (filter #(= :llm.routing/provider-retry (:event/type %)) (:routed/trace result))

              fb
              (first (filter #(= :llm.routing/provider-fallback (:event/type %))
                             (:routed/trace result)))]
  ...
  (it "`:on-chunk` from caller opts surfaces routing events live through `ask-code!`"
      ...
      (let [[clock _]
        ...
        (let [types (mapv :event/type @live-events)]
          (expect (= [:llm.routing/provider-retry :llm.routing/provider-fallback] types))
          (expect (= "p1" (:provider (first @live-events))))
          (expect (= "p2" (:to-provider (last @live-events))))))))

(defdescribe
  with-provider-fallback-all-exhausted-test
...
(defdescribe
  with-provider-fallback-single-provider-unavailable-test
  ...
  (it "throws :provider-unavailable with preserved status when the only provider fails"
      (let [[clock _]
            ...
            (llm/make-router
              [{:id :solo :api-key "k" :base-url "http://solo" :models [{:name "m1"}]}]
              {:clock clock :rate-limit {:same-provider-delays-ms [] :fallback-after-ms 0}})]
...
(defdescribe
  with-provider-fallback-non-429-transient-same-provider-retry-test
  ...
  (doseq [status [529 503 500 502 504]]
    (it
      (str "status " status " retries same provider then falls over to P2")
      (let [[clock _] (mock-clock)
...
(defdescribe
  with-provider-fallback-quota-billing-429-not-retried-test
...
(defdescribe
  with-provider-fallback-statusless-wrapper-transient-retried-test
  ...
  (doseq [msg ["Provider returned error" "upstream error: Service Unavailable"
               "RESOURCE_EXHAUSTED for model" "you can retry your request"]]
...
(defdescribe
  with-provider-fallback-client-4xx-transient-text-not-retried-test
...
(defdescribe
  circuit-breaker-open-on-threshold-test
...
(defdescribe
  circuit-breaker-half-open-cycle-test
  ...
  (it
    "open → half-open (time) → closed (probe success)"
    (let [[clock clock-atom]
          (mock-clock)
      ...
      (let [skipped (router/with-provider-fallback r
                                                   {:strategy :root}
                                                   (fn [_ _]
                                                     (success-result 10)))]
      ...
      (let [probe (router/with-provider-fallback r
                                                 {:strategy :root}
                                                 (fn [_ _]
                                                   (success-result 10)))]
        (expect (= :p1 (:routed/provider-id probe))))))
  (it
    "half-open → open on single probe failure"
    (let [[clock clock-atom]
          (mock-clock)
      ...
      (let [still-skipped (router/with-provider-fallback r
                                                         {:strategy :root}
                                                         (fn [_ _]
                                                           (success-result 10)))]
...
(defdescribe
  optimize-cost-by-real-pricing-test
...
(defdescribe
  ...
  (it "frontier in low-priority provider wins over high-tier in high-priority"
      (let [r (llm/make-router [;; priority 0: only `:high` model
  ...
  (it "provider priority tiebreaks WITHIN the same tier"
      (let [r (llm/make-router [;; both frontier — provider-a (priority 0) wins tiebreak
  ...
  (it "prepends Claude Opus 5 to existing subscription provider configs"
      (let [r
            (llm/make-router [{:id :anthropic-coding-plan
                               ...
                               :models [{:name "claude-opus-4-6"} {:name "claude-sonnet-4-6"}]}])
...
(defdescribe
  reasoning-filter-selects-capable-model-test
  ...
  (it "`:reasoning :deep` + `:optimize :cost` picks cheapest *reasoning-capable*"
      ...
      (let [r (llm/make-router [{:id :openai
                                 :api-key "k"
  ...
  (it "`:reasoning :deep` filters OUT a non-reasoning model even when it's cheapest"
      ...
      (let [r (llm/make-router [{:id :synthetic
                                 :api-key "k"
                                 :base-url "http://x"
                                 :models [{:name "cheap-dumb"
  ...
  (it "`:reasoning :deep` WITHOUT optimize routes the root model best-effort"
      ...
      (let [r (llm/make-router
                [{:id :openai :api-key "k" :models [{:name "gpt-4o"} {:name "gpt-4.1"}]}])]
...
(defn- seed-requests!
  ...
  (swap! (:state router) update-in
    [provider-id :requests]
    (fn [rs]
      (into (or rs []) (repeat n now)))))
...
(defdescribe
  catalog-rate-metadata-does-not-gate-test
  ...
  (it "ignores configured RPM when selecting a provider"
      (let [[clock clock-atom]
            ...
            r
            (llm/make-router
              [{:id :p1 :api-key "k" :base-url "http://p1" :rpm 3 :models [{:name "m1"}]}
               {:id :p2 :api-key "k" :base-url "http://p2" :rpm 3 :models [{:name "m2"}]}]
              {:clock clock})
            ...
            _
            (seed-requests! r :p1 @clock-atom 3)
  ...
  (it "ignores configured TPM when selecting a provider"
      (let [[clock clock-atom]
            ...
            r
            (llm/make-router
              [{:id :p1 :api-key "k" :base-url "http://p1" :tpm 100 :models [{:name "m1"}]}
               {:id :p2 :api-key "k" :base-url "http://p2" :tpm 100 :models [{:name "m2"}]}]
              {:clock clock})

            _
            (seed-tokens! r :p1 [{:ts @clock-atom :n 500}])]

        (expect (= :p1
                   (:routed/provider-id (router/with-provider-fallback r
                                                                       {}
                                                                       (fn [_ _]
                                                                         (success-result 10)))))))))
...
(defdescribe
  ...
  (it "throws when :max-tokens has been hit"
      (let [[clock _]
            ...
            r
            (llm/make-router [{:id :p1 :api-key "k" :base-url "http://p1" :models [{:name "m1"}]}]
                             {:clock clock :budget {:max-tokens 100}})]
  ...
  (it "throws when :max-cost has been hit (cost-exhausted message)"
      (let [[clock _]
            ...
            r
            (llm/make-router [{:id :p1 :api-key "k" :base-url "http://p1" :models [{:name "m1"}]}]
                             {:clock clock :budget {:max-cost 0.01}})]
  ...
  (it "does NOT throw when budget has headroom"
      (let [[clock _]
            ...
            r
            (llm/make-router [{:id :p1 :api-key "k" :base-url "http://p1" :models [{:name "m1"}]}]
                             {:clock clock :budget {:max-tokens 10000 :max-cost 10.0}})]
...
(defdescribe
  budget-record-accrues-tokens-and-cost-test
  ...
  (it "accrues token counts and USD cost after a successful call"
      (let [[clock _]
            ...
            r
            (llm/make-router [{:id :openai :api-key "k" :models [{:name "gpt-4o"}]}]
                             {:clock clock :budget {:max-tokens 10000000 :max-cost 100.0}})
        ...
        (let [expected-cost (+ (* (/ 1000.0 1e6) 2.50) (* (/ 2000.0 1e6) 10.00))]
  ...
  (it "router-stats exposes budget limit and spent"
      (let [[clock _]
            ...
            (router/with-provider-fallback
              ...
              {:strategy :root}
              (fn [_ _]
                {:result {} :api-usage {:input-tokens 100 :output-tokens 50 :total-tokens 150}}))
...
(defdescribe
  cb-429-vs-500-recovery-branch-test
  ...
  (let [infer #'router/regex-infer-metadata]
    ...
    (describe "`:frontier :slow` opus/frontier family"
              ...
              (it "frontier → frontier"
                  (expect (= :frontier (:intelligence (infer "acme-frontier-v1"))))))
    ...
    (describe "`:medium :medium` default"
              (it "unknown name"
                  (expect (= :medium (:intelligence (infer "totally-random-model"))))))
...
(defdescribe
  ask-with-reasoning-integration-test
  ...
  (it
    "reasoning filter reaches ask!'s provider selection"
    (let [captured
          (atom nil)
...
(defdescribe
  provider-native-reasoning-effort-routing-test
  (let [supported {:type "effort" :values ["high" "max"]}]
    (it "skips unsupported candidates and attaches actual resolution evidence"
        (let [r (llm/make-router
                  [{:id :synth
                    :api-key "k"
                    :base-url "http://x"
                    :models [{:name "binary-only" :reasoning? true :reasoning-style :zai-thinking}
                             {:name "glm-5.2"
                              :reasoning? true
              ...
              calls (atom [])
              result (router/with-provider-fallback r
                                                    {:strategy :root :reasoning-effort "max"}
                                                    (fn [_provider model]
          ...
          (expect (= {:requested "max"
                      ...
                      :extra-body {:thinking {:type "enabled"} :reasoning_effort "max"}}
        ...
        (let [r (llm/make-router
                  [{:id :p1
                   ...
                   {:id :p2
                    :api-key "k"
                    :base-url "http://2"
                    :models [{:name "binary" :reasoning? true :reasoning-style :zai-thinking}]}
                   {:id :p3
                    :api-key "k"
                    :base-url "http://3"
                    :models [{:name "glm-b"
              ...
              result (router/with-provider-fallback
                       r
                       {:strategy :root :reasoning-effort "high"}
                       (fn [provider _model]
                         (swap! calls conj (:id provider))
                         (if (= :p1 (:id provider)) (transient-error 503) (success-result 12))))]
    ...
    (it "rejects unsupported effort before invoking a provider"
        (let [r (llm/make-router [{:id :synth
                                   :api-key "k"
                                   :base-url "http://x"
                                   :models [{:name "glm-5.2"
                                             :reasoning? true
                                             ...
                                             :reasoning-options [supported]}]}])
              calls (atom 0)
              thrown (try (router/with-provider-fallback r
                                                         {:strategy :root
          ...
          (expect (= 0 @calls))
          (expect (= :svar/unsupported-reasoning-effort (:type (ex-data thrown))))
          (expect (= ["high" "max"] (:supported (ex-data thrown))))))))

(defdescribe
  rate-limited-provider-not-replayed-as-fallback-test
  ...
  (it
    "excludes a rate-limit-exhausted provider on the next fallback iteration"
    (let [[clock-fn clock-atom]
          (mock-clock)
          ...
          (fn [_p _m]
            ...
            (throw (ex-info "rate-limited" {:type :svar.core/http-error :status 429})))

          r
          (llm/make-router
            [{:id :test :api-key "k" :base-url "http://x" :models [{:name "only-model"}]}]
            {:clock clock-fn
          ...
          result
          (try (router/with-provider-fallback r {} f-429)
               (catch Exception e {:thrown-type (:type (ex-data e)) :tried (:tried (ex-data e))}))]
...
(defdescribe
  rate-limit-retry-respects-thread-interrupt-test
  ...
  (it
    "propagates InterruptedException out of the retry sleep instead of treating it as a retryable error"
    (let [worker-thread
          (atom nil)
          ...
          (fn [_p _m]
            ...
            (throw (ex-info "slow" {:type :svar.core/http-error :status 429})))

          r
          (llm/make-router [{:id :slow :api-key "k" :base-url "http://x" :models [{:name "m"}]}]
          ...
          worker
          (future (try (router/with-provider-fallback r {} slow-f)
                       :no-exception
                       (catch InterruptedException _ :interrupted)
                       (catch Exception e
                         {:class (.getSimpleName (class e))
                          :has-interrupt-cause? (boolean (loop [^Throwable t (.getCause e)]
                                                           (cond (nil? t) false
...
(defdescribe
  with-provider-fallback-model-unsupported-test
  "400/404 `model_not_supported` excludes the MODEL (not the provider) and
   retries a sibling — including on the same provider."
  (it
    "falls back to a sibling model on the SAME provider, excluding by name"
    (let [r
          (llm/make-router [{:id :copilot
                             :api-key "k"
                             :base-url "http://c"
  ...
  (it "every model unsupported → throws the concrete upstream reason"
      (let [r
            (llm/make-router
              [{:id :p1 :api-key "k" :base-url "http://p1" :models [{:name "m1"} {:name "m2"}]}])
...
(defdescribe
  ...
  (it "falls across providers after an auth rejection when explicitly enabled"
      (let [r
            ...
            (router/with-provider-fallback
              r
              {:on-auth-error :fallback-provider :on-chunk #(swap! live conj %)}
              (fn [provider model]
                (swap! calls conj [(:id provider) (:name model)])
                (if (= :p1 (:id provider))
      ...
      (let [r
            ...
            (router/with-provider-fallback
              ...
              (fn [provider model]
                (swap! calls conj [(:id provider) (:name model)])
                (if (= :p1 (:id provider))
                  (throw (ex-info "Token expired" {:type :svar.core/http-error :status 429}))
                  (success-result 100))))]
  ...
  (it "honors initial provider exclusions passed through public routing opts"
      (let [r
            ...
            {:keys [prefs]}
            (router/resolve-routing r {:exclude-providers #{:p1} :on-auth-error :fallback-provider})
...
(defdescribe
  ...
  (let [routed-name
        (fn [r prefs]
          (some-> (router/select-provider r prefs)
                  second
                  :name))

        mixed-fleet
        (fn []
          (llm/make-router [{:id :blind
                             :api-key "k"
                             :base-url "http://x"
    ...
    (it "carries :capabilities from routing opts into prefs"
        (let [r
              ...
              {:keys [prefs]}
              (router/resolve-routing r {:capabilities #{:vision} :optimize :cost})]
        ...
        (let [r
              ...
              {:keys [prefs]}
              (router/resolve-routing r {:optimize :cost})]
    ...
    (it "honors an explicit model pin over the capability filter"
        (let [r (mixed-fleet)]
          (expect (= "cheap-blind"
                     (routed-name r {:force-model "cheap-blind" :capabilities #{:vision}})))))
    (it "surfaces the routed model's capabilities to callers"
        (let [r
              (mixed-fleet)

              m
              (router/resolve-effective-model r {:capabilities #{:vision} :optimize :cost})]
    ...
    (it "finds the coding plan's only seeing model through prefer-providers"
        (let [r
              ...
              {:keys [prefs]}
              (router/resolve-routing r
                                      {:prefer-providers [:zai-coding-plan]
                                       :optimize [:cost :speed]
                                       :capabilities #{:vision}})]
    ...
    (it "answers what it would use INSTEAD when a provider is excluded"
        (let [r (mixed-fleet)]
          (expect (= "pricey-seer"
                     (:name (router/resolve-effective-model r
                                                            {:capabilities #{:vision}
                                                             :optimize :cost}))))
          (expect (nil? (router/resolve-effective-model r
                                                        {:capabilities #{:vision}
                                                         :optimize :cost
    ...
    (it
      "answers about the provider the caller PREFERS, not the cheapest one"
      (let [r (llm/make-router [{:id :cheap
                                 :api-key "k"
                                 :base-url "http://a"
        ...
        (expect
          (= "cheap-seer"
             (:name (router/resolve-effective-model r {:capabilities #{:vision} :optimize :cost}))))
        (expect (= "proven-seer"
                   (:name (router/resolve-effective-model r
                                                          {:capabilities #{:vision}
                                                           :optimize :cost
    ...
    (it
      "routes past ONE excluded model name without losing its provider"
      (let [r
            (llm/make-router [{:id :seeing
                               :api-key "k"
                               :base-url "http://y"
                               :models [{:name "seer-mini"
                                         :pricing {:input 0.10 :output 0.20}
                                         :intelligence :low
                                         :speed :fast
                                         :capabilities #{:chat :vision}}
            ...
            {:keys [prefs]}
            (router/resolve-routing
              r
              {:capabilities #{:vision} :optimize :cost :exclude-models #{"seer-mini"}})]
...
(defn- catalog-model-names
  ...
  [pid]
  (keys (modelsdev/provider-models (or (get-in router/KNOWN_PROVIDERS [pid :pricing-source])
...
(defdescribe
  provider-scoped-capability-test
  ...
  (describe
    ...
    (it "adds the vision a name heuristic misses"
        (doseq [[pid model] [[:openrouter "qwen/qwen2.5-vl-72b-instruct"]
                             [:mistral "mistral-medium-latest"] [:github-copilot "kimi-k2.7-code"]]]
    ...
    (it "reaches the same verdict the router's own normalization reaches"
        (doseq [pid
                [:openrouter :mistral :github-copilot :openai :anthropic :zai-coding-plan]
                ...
                model
                (take 40
                      (filter #(router/provider-model-visible? pid %) (catalog-model-names pid)))]

          (let [normalized (first (:models (router/normalize-provider
                                             0
                                             {:id pid :api-key "k" :models [{:name model}]})))]
            (expect (= (contains? (:capabilities normalized) :vision)
                       (contains? (:capabilities (router/provider-model-metadata pid {:name model}))
                                  :vision))
                    (str "one verdict for " pid "/" model))))))
  (describe
    "evidence stronger than the catalog"
    (it "honors capabilities the caller configured, both ways"
        ...
        (expect (contains? (:capabilities (router/provider-model-metadata
                                            :ollama
                                            {:name "llava:13b" :capabilities #{:chat :vision}}))
                           :vision))
        (expect (not (contains? (:capabilities (router/provider-model-metadata
                                                 :anthropic
                                                 {:name "claude-sonnet-5" :capabilities #{:chat}}))
                                :vision))))
    (it "honors the capability a provider's own /models body reported"
        ...
        (expect (contains? (:capabilities (router/provider-model-metadata :lmstudio
                                                                          {:name "some-local-vlm"
                                                                           :vision? true}))
                           :vision))
        (expect (not (contains? (:capabilities (router/provider-model-metadata
                                                 :openrouter
                                                 {:name "google/gemini-2.5-pro" :vision? false}))
                                :vision)))))
  (describe
    "a provider whose wire cannot carry pixels"
    ...
    (it "is inert when the flag is true or absent"
        (let [r (llm/make-router [{:id :gw
                                   ...
                                   :models [{:name "local-vlm" :capabilities #{:chat :vision}}]}])]
          (expect (= "local-vlm"
                     (:name (router/resolve-effective-model r {:capabilities #{:vision}}))))))
    (it "leaves every other capability alone"
        (let [r
              (llm/make-router [{:id :gw
                                 :api-key "k"
                                 :base-url "http://x"
                                 :image-input? false
                                 :models [{:name "local-vlm"
                                           :capabilities #{:chat :vision :reasoning}}]}])

              m
              (first (:models (first (:providers r))))]
  ...
  (describe "routing"
            (it "picks the catalog-sighted model over the one the name heuristic favors"
                (let [r (llm/make-router [{:id :openrouter
                                           :api-key "k"
