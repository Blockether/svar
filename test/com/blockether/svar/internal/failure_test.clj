...
(defdescribe
  transient-classification-test
  (describe
    "gateway wrapper text with no HTTP status"
    ...
    (it "retries LiteLLM's InternalServerError"
        (expect (:retryable? (sut/classify
                               (err "litellm.InternalServerError: Internal Server Error")))))
...
(defdescribe
  ...
  (describe "account state beats every transient signal"
            ...
            (it "never retries a billing failure"
                (expect (not (:retryable? (sut/classify (err
                                                          "billing: available balance is 0")))))))
  (describe "credentials"
            ...
            (it "classifies an invalid-api-key 400 as :auth, not invalid-request"
                (expect (= :auth
                           (:category (sut/classify (err "Invalid API key provided"
                                                         {:status 400}))))))
            (it "treats expired credentials as auth, even behind a transient status"
                (doseq [msg ["Token expired" "The access token has expired" "JWT token_expired"]]
...
(defdescribe
  ...
  (describe ":reached-model? answers 'can I safely send this again'"
            ...
            (it "is false for a connect-phase failure"
                (let [c (sut/classify
                          (ex-info "wrapped" {} (java.net.ConnectException. "Connection refused")))]
...
(defdescribe
  visible-output-test
  (describe
    "a call that already streamed visible output is never auto-retried"
    (it "refuses to retry a drop after content was emitted"
        (expect (not (:retryable? (sut/classify (err "stream connection error"
                                                     {:status 502 :content-acc-len 120}))))))
    (it "refuses to retry after reasoning was emitted"
        (expect (not (:retryable? (sut/classify (err "connection reset"
...
(defdescribe
  actionable-category-test
  (describe
    "categories a retry can never fix carry guidance instead"
    (it "names a model the endpoint does not serve"
        (let [c (sut/classify
                  (err "The model `gpt-5-turbo` does not exist or you do not have access to it"
                       {:status 404}))]
          (expect (= :model-unavailable (:category c)))
          (expect (not (:retryable? c)))
          (expect (string? (:next-step c)))))
    (it "names an Azure/Bedrock resource mismatch"
        (let [c (sut/classify (err "Item 'rs_abc' was not found for this resource" {:status 400}))]
    ...
    (it "keeps every category inside the published set"
        (expect (every? (fn [e]
                        ...
                        [(err "whatever") (err "boom" {:status 500}) (err "nope" {:status 401})
                         (err "billing") (err "timeout" {:status 504})])))))
  ...
  (describe "request-id"
                ...
                (expect
                  (some?
                    (sut/request-id
                      (err "boom"
                           {:body
                            "Internal error. Request ID: 5f0a1c2e-1111-4222-8333-abcdef012345"})))))
            ...
            (it "is carried on every classification"
                (expect (= "req_1"
                           (:request-id
                             (sut/classify
                               (err "boom" {:status 500 :headers {"x-request-id" "req_1"}}))))))))
  ...
  (describe
    ...
    (it "ignores an unparsable HTTP-date Retry-After rather than sleeping nonsense"
        (expect (= {:delay-ms 1000}
                   (sut/retry-sleep-plan
                     (err "slow down" {:headers {"retry-after" "Wed, 21 Oct 2015 07:28:00 GMT"}})
                     {:fallback-ms 1000}))))
    ...
    (it "is spent once the budget is gone"
        (expect (nil? (sut/retry-sleep-plan
                        (err "boom" {:status 503})
...
(defdescribe
  transport-retryable-test
  (describe
    "soft transport drops"
    ...
    (it "retries the LiteLLM/tunnel 'received no bytes' pre-response drop"
        (expect (sut/transport-retryable? (ex-info "HTTP/1.1 header parser received no bytes" {}))))
    (it "retries a mid-stream connection reset"
        (expect (sut/transport-retryable? (err "stream connection error" {:stream? true}))))
    (it "retries a transient OS/network blip through the cause chain"
        (expect (sut/transport-retryable?
                  (ex-info "wrapped" {} (java.net.SocketException. "Network is unreachable")))))
  ...
  (describe "hard / deliberate aborts are never retried"
            (it "refuses svar's own idle-watchdog abort"
                (expect (false? (sut/transport-retryable? (err "Stream closed"
            ...
            (it "refuses a plain application error"
                (expect (false? (sut/transport-retryable? (err "invalid api key"
                                                               {:status 401})))))))

(defdescribe
  host-connect-health-test
  (describe
    "message-less ConnectException gate"
    (it "is NOT retryable for a host never reached"
        (reset! sut/host-connect-health* {})
        (expect (false? (sut/healthy-host-connect-blip? (ex-info
                                                          ""
                                                          {:url "http://never-reached.test:1234/v1"}
    ...
    (it "IS retryable right after that same host connected successfully"
        ...
        (expect (sut/healthy-host-connect-blip?
                  (ex-info "" {:url "http://healthy.test:4321/v1"} (java.net.ConnectException.)))))
    (it "expires outside the health window"
        (reset! sut/host-connect-health* {"stale.test" (- (System/currentTimeMillis)
                                                          (inc (long
                                                                 sut/CONNECT_HEALTH_WINDOW_MS)))})
...
(defdescribe
  ...
  (describe "connect-phase detection + human message"
            (it "sees a ConnectException anywhere in the chain"
                (expect (sut/connection-error?
                          (ex-info "wrapped" {} (java.net.ConnectException. "refused")))))
            ...
            (it "names the provider host and keeps the cause"
                (let [e (sut/connection-error->ex-info
                          (java.net.ConnectException.)
...
(defdescribe
  unanswered-request-retry-test
  (describe
    "a request the model never saw is replayed, with or without a status"
    ...
    (it "retries LiteLLM's 408 connect timeout carried in the body"
        (let [e
              ...
              {:keys [retry? reason classification]}
    ...
    (it "leaves a status-less drop to transport-retryable? and its host-health gate"
        ...
        (let [e (err "gateway error"
                     {:type :svar.core/http-error
                      :body
                      "litellm.APIConnectionError: HTTP/1.1 header parser received no bytes"})]
...
(defdescribe
  no-retry-reason-test
  (describe
    "a refusal carries the reason it refused"
    (it "names the streamed output that makes a truncated stream terminal"
        (let [e
              (err "Stream ended before terminal marker."
                   {:type :svar.core/stream-truncated :stream? true :reasoning-acc-len 4200})

              {:keys [retry? no-retry-reason classification]}
              (sut/low-level-retry-decision e)]
...
(defdescribe
  router-transient-error-test
  (describe
    "the router's broader soft/hard verdict"
    ...
    (it "never re-routes a hard quota/billing wall, whatever the status"
        (expect (false? (sut/transient-error? (err "Your credit balance is too low"
                                                   {:status 429})))))
    (it "never re-routes a definitive client error"
        (expect (false? (sut/transient-error? (err "invalid request: bad tool schema"
                                                   {:status 400})))))))

(defdescribe
  single-source-of-truth-test
  (describe
    "no layer keeps its own copy of the heuristics"
    (it "llm's connect-phase check IS failure/connection-error?"
        (expect (identical? sut/connection-error?
                            @(requiring-resolve
                               'com.blockether.svar.internal.llm/connection-error?))))
    (it "the router's watchdog set IS failure/STREAM_WATCHDOG_ERROR_TYPES"
        (expect (identical? sut/STREAM_WATCHDOG_ERROR_TYPES
                            @(requiring-resolve
                               'com.blockether.svar.internal.router/STREAM_WATCHDOG_ERROR_TYPES))))
    ...
    (it "the router's auth/model policy IS failure/classify"
        (expect (identical? sut/classify
                            @(requiring-resolve
                               'com.blockether.svar.internal.router/classify-failure))))))

(defdescribe
  budget-wall-is-hard-test
  (describe
    "every phrasing of a quota/credit/budget wall"
    (it "is a hard :quota-exhausted failure even on a 429"
        (doseq [msg ["Your budget has been exceeded" "ExceededBudget: Crossed spend within budget"
                     "Budget exceeded for key sk-***" "Max budget reached for this key"
                     "Monthly spend limit reached"
                     "Your credit balance is too low to run this request"
                     "Insufficient credits — add credits to continue"
                     "You exceeded your current quota, please check your plan" "Payment Required"]]
    ...
    (it
      "surfaces Anthropic extra-usage exhaustion without one retry"
      (let
        [body
    ...
    (it "treats a bare HTTP 402 as the same account wall"
        (let [e
              (ex-info "Exceptional status code: 402" {:type :svar.core/http-error :status 402})
...
(defdescribe
  routed-attempt-classification-test
  ...
  (it
    "classifies an all-auth fleet from routing attempts alone"
    (let [e
          (ex-info
            "All providers exhausted"
            {:type :svar.llm/all-providers-exhausted
             :attempts
             [{:provider :a :status 401 :reason :authentication :error "API authentication failed"}
              {:provider :b :status 401 :reason :authentication :error "Invalid API key"}]})
