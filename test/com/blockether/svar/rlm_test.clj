...
(defdescribe disposable-db-test
  ...
  (describe "wrap-external-db"
            (it "wraps external connection with owned? false"
                (let [path (#'sut/create-temp-db-path)
                      external-conn (d/create-conn path {})
                      db-info (#'sut/wrap-external-db external-conn)]
                  (try
                    (expect (= external-conn (:conn db-info)))
                    (expect (false? (:owned? db-info)))
                    (finally
                      (d/close external-conn)
                      (fs/delete-tree path))))))

  (describe "dispose-db!"
            (it "disposes owned database"
                (let [db-info (#'sut/create-disposable-db)
                      path (:path db-info)]
...
;; =============================================================================
;; Database SCI Bindings Tests
;; =============================================================================

(defdescribe database-sci-bindings-test
  (describe "db-schema!"
            (it "creates schema in disposable db"
                (with-test-env* {} (fn [env]
                  (let [result (#'sut/execute-code env "(db-schema! {:user/name {}})")]
                    (expect (= :schema-updated (:result result)))
                    (expect (nil? (:error result))))))))

  (describe "db-transact!"
            (it "transacts data to disposable db"
                (with-test-env* {} (fn [env]
                  (#'sut/execute-code env "(db-schema! {:user/name {}})")
                  (let [result (#'sut/execute-code env "(db-transact! [{:user/name \"Alice\"}])")]
                    (expect (= :transacted (:result result)))
                    (expect (nil? (:error result))))))))

  (describe "db-q"
            (it "queries data from disposable db"
                (with-test-env* {} (fn [env]
                  (#'sut/execute-code env "(db-schema! {:user/name {}})")
                  (#'sut/execute-code env "(db-transact! [{:user/name \"Alice\"} {:user/name \"Bob\"}])")
                  (let [result (#'sut/execute-code env "(db-q '[:find ?n :where [?e :user/name ?n]])")]
                    (expect (nil? (:error result)))
                    (expect (= #{["Alice"] ["Bob"]} (set (:result result)))))))))

  (describe "database errors when disabled"
            (it "throws error when db is disabled"
                (with-test-env* {} {:db false} (fn [env]
                  (let [result (#'sut/execute-code env "(db-q '[:find ?e :where [?e :user/name]])")]
          ;; Should error because db-q is not available when db is disabled
                    (expect (some? (:error result)))))))))
...
(defdescribe learnings-test
  ...
  (describe "db-increment-applied-count!"
          ...
          ;; Search twice - verify results are returned so we know tracking should work
                    (let [results1 (#'sut/db-get-learnings db-info "test insight tracking" {:top-k 5})
                          results2 (#'sut/db-get-learnings db-info "test insight tracking" {:top-k 5})]
...
(def ^:private test-config
  "LLM config for integration tests. 
   Uses OPENAI_API_KEY or BLOCKETHER_LLM_API_KEY env var."
...
(defmacro with-integration-env
  "Creates an RLM environment for integration tests, executes body, then disposes."
  [[env-sym] & body]
  `(let [~env-sym (sut/create-env {:config test-config})]
     (try
       ~@body
       (finally
         (sut/dispose! ~env-sym)))))
...
(defdescribe entity-schema-test
  (it "creates DB and stores entity without embedding"
      (with-test-env* {} (fn [env]
        (let [db-info @(:db-info-atom env)
              conn (:conn db-info)
        ...
        ;; Query it back
          (let [result (d/q '[:find ?e ?name ?type
                              :where
                              [?e :entity/id ?id]
                              [?e :entity/name ?name]
                              [?e :entity/type ?type]]
            ...
            (expect (= ["John Doe" :party] (rest (first result)))))))))
  ...
  (it "stores and retrieves entity with legal extension attributes"
      (with-test-env* {} (fn [env]
        ...
        ;; Transact entity with legal attributes
          (d/transact! conn [entity])
        ;; Query it back
          (let [result (d/q '[:find ?role ?eff ?exp
                              :where
                              [?e :entity/id ?id]
                              [?e :legal/party-role ?role]
                              [?e :legal/effective-date ?eff]
                              [?e :legal/expiry-date ?exp]]
                            (d/db conn))]
            (expect (= 1 (count result)))
            (expect (= [:contractor "2025-01-01" "2026-01-01"] (first result))))))))
  ...
  (it "stores and retrieves relationship between two entities"
      (with-test-env* {} (fn [env]
        ...
        ;; Transact both entities and relationship
          (d/transact! conn [source-entity target-entity relationship])
        ;; Query relationship
          (let [result (d/q '[:find ?src ?tgt ?type ?desc
                              :where
                              [?r :relationship/id ?id]
                              [?r :relationship/source-entity-id ?src]
                              [?r :relationship/target-entity-id ?tgt]
                              [?r :relationship/type ?type]
                              [?r :relationship/description ?desc]]
                            (d/db conn))]
            (expect (= 1 (count result)))
            (let [[src tgt type desc] (first result)]
              (expect (= source-id src))
              (expect (= target-id tgt))
              (expect (= :obligates type))
              (expect (= "Party A is obligated to perform" desc))))))))
  ...
  (it "stores and retrieves claim with citation and verification verdict"
      (with-test-env* {} (fn [env]
        ...
        ;; Query it back
          (let [result (d/q '[:find ?text ?quote ?conf ?verdict ?verified
                              :where
                              [?c :claim/id ?id]
                              [?c :claim/text ?text]
                              [?c :claim/quote ?quote]
                              [?c :claim/confidence ?conf]
                              [?c :claim/verification-verdict ?verdict]
                              [?c :claim/verified? ?verified]]
                            (d/db conn))]
            (expect (= 1 (count result)))
            (let [[text quote conf verdict verified] (first result)]
              (expect (= "The contract is valid" text))
              (expect (= "This contract is valid and binding" quote))
              (expect (= (float 0.95) conf))
              (expect (= "correct" verdict))
              (expect (true? verified)))))))))
...
(defdescribe knowledge-engine-real-llm-test
  ...
  (describe "query on multi-page document with real LLM"
          ...
          ;; Ingest multi-page document first
            ...
            ;; Consensus efficiency: medium query should finish within 5 iterations
